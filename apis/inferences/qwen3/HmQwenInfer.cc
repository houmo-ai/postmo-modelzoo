...
HmQwenInfer::HmQwenInfer(const std::string &prefillModelPath,
  ...
  // tokenizer init
  tokenizer = std::make_shared<HmTokenizer>(
      tokenizerJsonPath, embeddingWeightPath, this->embedding_length,
      this->prefill_length);
...
void HmQwenInfer::DebugModelInfo(tcim::Module &module,
...
void HmQwenInfer::PrefillSetInputDatas(void *data, int32_t valid_length,
...
void HmQwenInfer::Chat(const std::string &msg) {
  ...
  if (input_echo_len > context_max_length) {
    std::cerr << "Question long than " << context_max_length
