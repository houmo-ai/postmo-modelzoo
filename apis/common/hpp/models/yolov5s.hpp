...
inline int non_max_suppression(std::vector<DetectResult> &detections,
                               const float iou_threshold) {
  // sort
  std::sort(detections.begin(), detections.end(),
            [](const DetectResult &d1, const DetectResult &d2) {
...
class YoloV5 {
 public:
  std::vector<DetectResult> postprocess(const cv::Mat &image,
        ...
        for (int w = 0; w <= feat_w - 1; w++) {
          for (int a = 0; a <= anchor_num - 1; a++) {
            ...
            for (int s = 0; s <= cls_num - 1; s++) {
              float score =
                feat[a * feat_w * feat_h * (cls_num + 5) +
                     h * feat_w * (cls_num + 5) + w * (cls_num + 5) + s + 5];
            ...
            float box_score =
              feat[a * feat_w * feat_h * (cls_num + 5) +
                   (h * feat_w) * (cls_num + 5) + w * (cls_num + 5) + 4];
            float final_score = box_score * class_score;
            if (final_score >= conf_threshold_) {
              int loc_idx = a * feat_h * feat_w * (cls_num + 5) +
                            h * feat_w * (cls_num + 5) + w * (cls_num + 5);
  ...
  std::vector<DetectResult> postprocess(const int height, const int width,
                                        std::vector<DetectOutput> outputs) {
    std::vector<DetectResult> detections;
    static float anchors[18] = {10, 13, 16,  30,  33, 23,  30,  61,  62,
                                45, 59, 119, 116, 90, 156, 198, 373, 326};

    float scale = (float)input_sizes_[0] / std::max(height, width);
    float pad_h = (input_sizes_[0] - height * scale) * 0.5f;
    float pad_w = (input_sizes_[1] - width * scale) * 0.5f;
