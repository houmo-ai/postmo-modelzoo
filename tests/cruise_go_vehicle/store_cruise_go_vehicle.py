...
def get_onnx_module():
    if not os.path.exists('cruise_cutin_vehicle.zip'):
        os.system('curl -upublic:Password@123 \
            http://10.10.1.53:8082/artifactory/hdpl_test_data/quant_models/cruise_go_vehicle.zip \
            -o cruise_go_vehicle.zip')
    if not os.path.exists('cruise_go_vehicle'):
        os.system('unzip cruise_go_vehicle.zip')
    return "./cruise_go_vehicle/hmquant_cruise_go_vehicle_model.3D_shape_namecorrect_quant.onnx_with_act.onnx"
