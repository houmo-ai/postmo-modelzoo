...
# Description:
#   Model Download And Run llm_perf tool Script for Houmo AI LLM and VLM models.
#
...
all_test_models = [
    f"{TARGET_PATH}/deepseek/hmm_xh2_deepseek-8b_256_32k_b1_1chip_2cores_v{VERSION}.zip",
    f"{TARGET_PATH}/qwen2.5/hmm_xh2_qwen2.5-7b_256_8k_b1_1chip_2cores_v{VERSION}.zip",
    f"{TARGET_PATH}/qwen2.5-vl/hmm_xh2_qwen2.5-vl-7b_256_8k_b1_1chip_2cores_v{VERSION}.zip",
    f"{TARGET_PATH}/qwen3/hmm_xh2_qwen3-8b_256_16k_b4_1chip_2cores_v{VERSION}.zip",
    f"{TARGET_PATH}/qwen3-vl/hmm_xh2_qwen3-vl-8b_256_32k_b1_1chip_2cores_v{VERSION}.zip",
]
...
def parse_hmm_zip_filename(
    ...
    if isinstance(filename, list):
        ...
        raise TypeError(
            "Input must be str (single filename) or list (multiple filenames)"
        )
...
if __name__ == "__main__":
    ...
    for idx, model_url in enumerate(all_test_models):
        ...
        relative_model_paths[sub_model_dir_name] = {
            ...
            "batch": res["batch"],
            "ndevices": (
                res["ndevice"].replace("chips", "")
                if "chips" in res["ndevice"]
                else res["ndevice"].replace("chip", "")
            ),
            "loop": "1",
            "input": str(DECODE_STOP),
            "output": (
                str(DECODE_STOP) if context_length > DECODE_STOP else context_length
            ),
        }
        if int(relative_model_paths[sub_model_dir_name]["ndevices"]) > total_device_num:
            continue
        logger.info(
            f"Local Model Path: {relative_model_paths[sub_model_dir_name]['model_path']}"
        )
        get_file_from_jfrog(
            relative_model_paths[sub_model_dir_name]["download_url"],
            zip_dir,
            relative_model_paths[sub_model_dir_name]["model_path"],
        )
        logger.info(
            f"Local dirs files is {os.listdir(relative_model_paths[sub_model_dir_name]['model_path'])}, prefill file is {find_local(relative_model_paths[sub_model_dir_name]['model_path'], 'prefill')}"
        )
        prefill_path = os.path.join(
            relative_model_paths[sub_model_dir_name]["model_path"],
            find_local(
                relative_model_paths[sub_model_dir_name]["model_path"], "prefill"
            ),
        )
        decode_path = os.path.join(
            relative_model_paths[sub_model_dir_name]["model_path"],
            find_local(
                relative_model_paths[sub_model_dir_name]["model_path"], "decode"
            ),
        )
        visual_path = find_local(
            relative_model_paths[sub_model_dir_name]["model_path"], "visual"
        )
        if visual_path is not None:
            visual_path = os.path.join(
                relative_model_paths[sub_model_dir_name]["model_path"], visual_path
            )
        embedding_path = os.path.join(
            relative_model_paths[sub_model_dir_name]["model_path"],
            "hmquant/quant_embedding.pt",
        )
        type = "llm" if "vl" not in sub_model_dir_name else "vlm"
        ...
        perf_config["embedding"] = os.path.join(
            relative_model_paths[sub_model_dir_name]["model_path"],
            "hmquant/quant_embedding.bin",
        )
