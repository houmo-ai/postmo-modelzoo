...
class HmQwen:
    ...
    def __init__(self, model_dir, prefill_length, decode_length, batch=1, nblocks=28):
        ...
        self.prefill_model_part1 = tcim.runtime.load(os.path.join(model_dir, "deepseek_prefill_part1.hmm"), option = option1_0)
        self.prefill_model_part2 = tcim.runtime.load(os.path.join(model_dir, "deepseek_prefill_part2.hmm"), option = option1_1)
        ...
        for i in range(nblocks // 2, nblocks):
            ...
            self.decode_model_part2.set_input(f'model_layers_{i}_self_attn_vcache_input', vcache)
            kcache_history_sum = self.prefill_model_part2.get_input(f'model_layers_{i}_self_attn_kcache_history_sum')
            self.decode_model_part2.set_input(f'model_layers_{i}_self_attn_kcache_history_sum', kcache_history_sum)
        # set decode input
    ...
    def chat(self, question):
        ...
        input_data = self.prefill_model_part2.get_output("Output_lm_head_add_list_1").numpy()
        ...
        while True:
            ...
            input_data = self.decode_model_part2.get_output("Output_lm_head_add_list_1").numpy()
            decode_count += 1
