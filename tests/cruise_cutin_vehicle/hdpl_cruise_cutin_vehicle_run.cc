...
/*!
 * \file hdpl_cruise_cutin_vehicle_run.cc
 */
#include <unistd.h>
#include <cassert>
#include <iostream>
...
void RunGolden(runtime::NDArray output, std::string path, std::string file) {
  file = path + "hmquant_cruise_cutin_vehicle_model.3D_shape_namecorrect_quant.onnx_with_act/" + file + "_npy.txt";
...
void HDPLRuntimeLightRecog() {
  ...
  int size = 1 * 148;
  ReadDataFromFile("cruise_cutin_vehicle/hmquant_cruise_cutin_vehicle_model.3D_shape_namecorrect_quant.onnx_input_npy.txt", pA, size);

  auto module = tvm::hdpl::LoadModelPackage("./libcruise_cutin_vehicle");
  module.SetInput("input", feature);
  module.Run();
  RunGolden(module.GetOutput(0), "cruise_cutin_vehicle/", "prob");
  RunGolden(module.GetOutput(1), "cruise_cutin_vehicle/", "time_to_center");
}
int main() {
  std::string module = "libcruise_cutin_vehicle.so";
