import time
from hmodel.thirdparty.YOLOP.lib.core.evaluate import ConfusionMatrix, SegmentationMetric
from hmodel.thirdparty.YOLOP.lib.core.general import (
    non_max_suppression,
    check_img_size,
    scale_coords,
    xyxy2xywh,
...
from hmodel.thirdparty.YOLOP.lib.utils.utils import time_synchronized
from hmodel.thirdparty.YOLOP.lib.utils import plot_img_and_mask, plot_one_box, show_seg_result
import torch
...
from hmquant.qat_torch.quantizer import UniformQuantizer

device = torch.device("cuda:0")


class TrainInfo:
    def __init__(self, anchors, gr=1, nc=1) -> None:
...
anchor_grid = torch.from_numpy(anchors).to(device)
...
for i in range(len(stride)):
    ...
    grid.append(torch.from_numpy(make_grid(w, h)).to(device))

traininfo = TrainInfo(anchors)
...
def _pre(outputs):
    ...
    for i, out in enumerate(outputs[0]):
        ...
        bs = out.shape[0]
        y = out.sigmoid()
        y[..., 0:2] = (y[..., 0:2] * 2.0 - 0.5 + grid[i].to(out.device)) * stride[i]  # xy
        y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * anchor_grid[i]  # wh
        z.append(y.view(bs, -1, no))
    outputs[0] = (torch.cat(z, 1).to(out.device),outputs[0])
    return outputs
...
def train(
    ...
    for i, (input, target, paths, shapes) in enumerate(train_loader):
        ...
        outputs = model(input)
        total_loss, head_losses = criterion(outputs, target, shapes, raw_model)
        # print(head_losses)
        ...
        if rank in [-1, 0]:
            ...
            if i % cfg.PRINT_FREQ == 0:
                ...
                #     if isinstance(m, UniformQuantizer) and m.calibrated:
                #         writer.add_scalar(
                #             "Quantizer/{}.scale".format(n),
                #             m.learnable_scale.view(-1)[0].item(),
                #             global_steps,
                #         )
                #     if isinstance(m, nn.Conv2d):
                #         writer.add_scalar(
                #             "Conv/{}.w".format(n),
                #             m.weight.view(-1)[0].item(),
                #             global_steps,
                #         )
...
def validate(
    ...
    seen = 0
    confusion_matrix = ConfusionMatrix(nc=raw_model.nc)  # detector confusion matrix
    ...
    names = {k: v for k, v in enumerate(raw_model.names if hasattr(raw_model, "names") else raw_model.module.names)}
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        with torch.no_grad():
            ...
            total_loss, head_losses = criterion(
                (train_out, da_seg_out, ll_seg_out), target, shapes, raw_model
            )  # Compute loss
