...
class XHGemma4Processor(Gemma4Processor):
    ...
    def _chunk_long_audio_inputs(self, model_inputs):
        ...
        feature_ranges = []
        input_indices = []
        for input_index in range(input_features.shape[0]):
            total_frames = int(
                input_features_mask[input_index]
                .to(torch.int64)
                .sum()
                .item()
            )
            slices = self._build_audio_chunk_slices(
                total_frames, feature_length, stride
            )
            input_keep_ranges = self._build_audio_chunk_keep_ranges(
                slices, total_frames
            )
            for (start, end), keep_range in zip(
                slices, input_keep_ranges
            ):
                feature_chunk = input_features[input_index, start:end, :]
                mask_chunk = input_features_mask[input_index, start:end]
                ...
                feature_ranges.append((start, end))
        ...
        model_inputs["audio_chunk_feature_ranges"] = torch.tensor(feature_ranges, dtype=torch.int32)
        model_inputs["audio_chunk_input_indices"] = torch.tensor(input_indices, dtype=torch.int32)
    ...
    def _retokenize_audio_placeholders_if_needed(self, model_inputs, kwargs):
        ...
        if audio_chunk_output_ranges is not None:
            ...
            input_indices = model_inputs.get("audio_chunk_input_indices")
            if input_indices is None:
                expected_counts = [
                    int((ranges[:, 1] - ranges[:, 0]).sum().item())
                ]
            else:
                input_indices = torch.as_tensor(
                    input_indices, dtype=torch.long
                )
                ...
                expected_counts = [
                    int(
                        (
                            ranges[input_indices == input_index, 1]
                            - ranges[input_indices == input_index, 0]
                        )
                        .sum()
                        .item()
                    )
                    for input_index in range(
                        int(input_indices.max().item()) + 1
                    )
                ]
        else:
            expected_counts = [
