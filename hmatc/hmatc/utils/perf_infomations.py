...
class PERFTYPE(IntEnum):
    ...
    DECODE_TOTAL_TIME = 11
    DECODE_TOKEN_TIME = 12
    DECODE_EMBED_TIME = 13
    DECODE_INPUT_TIME = 14
    DECODE_INFER_TIME = 15
    DECODE_OUTPUT_TIME = 16

    # Vision processing times
    VISION_TOTAL_TIME = 20
    VISION_PREPROCESS_TIME = 21
    VISION_INPUT_TIME = 22
    VISION_INFER_TIME = 23
    VISION_OUTPUT_TIME = 24

    # audio
    AUDIO_TOTAL_TIME = 30
    AUDIO_PREPROCESS_TIME = 31
    AUDIO_INPUT_TIME = 32
    AUDIO_INFER_TIME = 33
    AUDIO_OUTPUT_TIME = 34

    PREFILL_LOAD_TIME = 1000
    DECODE_LOAD_TIME = 1001
    VISION_LOAD_TIME = 1002
    AUDIO_LOAD_TIME = 1003
...
class PerfInformations:
    ...
    # audio
    audio_total_time: float = 0.0
    audio_preprocess_time: float = 0.0
    audio_setinput_time: float = 0.0
    audio_infer_time: float = 0.0
    audio_getoutput_time: float = 0.0
    ...
    # audio
    audio_total_speed: float = 0.0
    audio_preprocess_speed: float = 0.0
    audio_infer_speed: float = 0.0
...
class InferenceMetrics:
    ...
    vision_perf_infos: PerfInformations = field(default_factory=PerfInformations)
    audio_perf_infos: PerfInformations = field(default_factory=PerfInformations)
    ...
    audio_load_time_start: float = 0.0
...
class InferencePerformanceTracker:
    ...
    Core functions:
    1. Record Prefill/Decode/Vision/Audio phase time consumption and token/image counts
    ...
    def __init__(self):
        ...
        self.start_time_mapping = {
            PERFTYPE.PREFILL_TOTAL_TIME: lambda: setattr(
                self.current_metrics.prefill_perf_infos, "total_time_start", time.time()
            ),
            PERFTYPE.PREFILL_TOKEN_TIME: lambda: setattr(
                self.current_metrics.prefill_perf_infos,
                "tokenizer_time_start",
                time.time(),
            ),
            PERFTYPE.PREFILL_EMBED_TIME: lambda: setattr(
                self.current_metrics.prefill_perf_infos,
                "embedding_time_start",
                time.time(),
            ),
            PERFTYPE.PREFILL_INPUT_TIME: lambda: setattr(
                self.current_metrics.prefill_perf_infos,
                "setinput_time_start",
                time.time(),
            ),
            PERFTYPE.PREFILL_INFER_TIME: lambda: setattr(
                self.current_metrics.prefill_perf_infos, "infer_time_start", time.time()
            ),
            PERFTYPE.PREFILL_OUTPUT_TIME: lambda: setattr(
                self.current_metrics.prefill_perf_infos,
                "getoutput_time_start",
                time.time(),
            ),
            PERFTYPE.DECODE_TOTAL_TIME: lambda: setattr(
                self.current_metrics.decode_perf_infos, "total_time_start", time.time()
            ),
            PERFTYPE.DECODE_TOKEN_TIME: lambda: setattr(
                self.current_metrics.decode_perf_infos,
                "tokenizer_time_start",
                time.time(),
            ),
            PERFTYPE.DECODE_EMBED_TIME: lambda: setattr(
                self.current_metrics.decode_perf_infos,
                "embedding_time_start",
                time.time(),
            ),
            PERFTYPE.DECODE_INPUT_TIME: lambda: setattr(
                self.current_metrics.decode_perf_infos,
                "setinput_time_start",
                time.time(),
            ),
            PERFTYPE.DECODE_INFER_TIME: lambda: setattr(
                self.current_metrics.decode_perf_infos, "infer_time_start", time.time()
            ),
            PERFTYPE.DECODE_OUTPUT_TIME: lambda: setattr(
                self.current_metrics.decode_perf_infos,
                "getoutput_time_start",
                time.time(),
            ),
            PERFTYPE.VISION_TOTAL_TIME: lambda: setattr(
                self.current_metrics.vision_perf_infos, "total_time_start", time.time()
            ),
            PERFTYPE.VISION_PREPROCESS_TIME: lambda: setattr(
                self.current_metrics.vision_perf_infos,
                "vision_preprocess_time_start",
                time.time(),
            ),
            PERFTYPE.VISION_INPUT_TIME: lambda: setattr(
                self.current_metrics.vision_perf_infos,
                "setinput_time_start",
                time.time(),
            ),
            PERFTYPE.VISION_INFER_TIME: lambda: setattr(
                self.current_metrics.vision_perf_infos, "infer_time_start", time.time()
            ),
            PERFTYPE.VISION_OUTPUT_TIME: lambda: setattr(
                self.current_metrics.vision_perf_infos,
                "getoutput_time_start",
                time.time(),
            ),
            PERFTYPE.AUDIO_TOTAL_TIME: lambda: setattr(
                self.current_metrics.audio_perf_infos, "total_time_start", time.time()
            ),
            PERFTYPE.AUDIO_PREPROCESS_TIME: lambda: setattr(
                self.current_metrics.audio_perf_infos,
                "audio_preprocess_time_start",
                time.time(),
            ),
            PERFTYPE.AUDIO_INPUT_TIME: lambda: setattr(
                self.current_metrics.audio_perf_infos,
                "setinput_time_start",
                time.time(),
            ),
            PERFTYPE.AUDIO_INFER_TIME: lambda: setattr(
                self.current_metrics.audio_perf_infos, "infer_time_start", time.time()
            ),
            PERFTYPE.AUDIO_OUTPUT_TIME: lambda: setattr(
                self.current_metrics.audio_perf_infos,
                "getoutput_time_start",
                time.time(),
            ),
            PERFTYPE.PREFILL_LOAD_TIME: lambda: setattr(
                self.current_metrics, "prefill_load_time_start", time.time()
            ),
            PERFTYPE.DECODE_LOAD_TIME: lambda: setattr(
                self.current_metrics, "decode_load_time_start", time.time()
            ),
            PERFTYPE.VISION_LOAD_TIME: lambda: setattr(
                self.current_metrics, "vision_load_time_start", time.time()
            ),
            PERFTYPE.AUDIO_LOAD_TIME: lambda: setattr(
                self.current_metrics, "audio_load_time_start", time.time()
            ),
        }

        # Mapping for perf_end operations
        self.end_time_mapping = {
            PERFTYPE.PREFILL_TOTAL_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.prefill_perf_infos,
                "total_time",
                getattr(self.current_metrics.prefill_perf_infos, "total_time")
                + time_diff_ms,
            ),
            PERFTYPE.PREFILL_TOKEN_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.prefill_perf_infos,
                "tokenizer_time",
                getattr(self.current_metrics.prefill_perf_infos, "tokenizer_time")
                + time_diff_ms,
            ),
            PERFTYPE.PREFILL_EMBED_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.prefill_perf_infos,
                "embedding_time",
                getattr(self.current_metrics.prefill_perf_infos, "embedding_time")
                + time_diff_ms,
            ),
            PERFTYPE.PREFILL_INPUT_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.prefill_perf_infos,
                "setinput_time",
                getattr(self.current_metrics.prefill_perf_infos, "setinput_time")
                + time_diff_ms,
            ),
            PERFTYPE.PREFILL_INFER_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.prefill_perf_infos,
                "infer_time",
                getattr(self.current_metrics.prefill_perf_infos, "infer_time")
                + time_diff_ms,
            ),
            PERFTYPE.PREFILL_OUTPUT_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.prefill_perf_infos,
                "getoutput_time",
                getattr(self.current_metrics.prefill_perf_infos, "getoutput_time")
                + time_diff_ms,
            ),
            PERFTYPE.DECODE_TOTAL_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.decode_perf_infos,
                "total_time",
                getattr(self.current_metrics.decode_perf_infos, "total_time")
                + time_diff_ms,
            ),
            PERFTYPE.DECODE_TOKEN_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.decode_perf_infos,
                "tokenizer_time",
                getattr(self.current_metrics.decode_perf_infos, "tokenizer_time")
                + time_diff_ms,
            ),
            PERFTYPE.DECODE_EMBED_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.decode_perf_infos,
                "embedding_time",
                getattr(self.current_metrics.decode_perf_infos, "embedding_time")
                + time_diff_ms,
            ),
            PERFTYPE.DECODE_INPUT_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.decode_perf_infos,
                "setinput_time",
                getattr(self.current_metrics.decode_perf_infos, "setinput_time")
                + time_diff_ms,
            ),
            PERFTYPE.DECODE_INFER_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.decode_perf_infos,
                "infer_time",
                getattr(self.current_metrics.decode_perf_infos, "infer_time")
                + time_diff_ms,
            ),
            PERFTYPE.DECODE_OUTPUT_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.decode_perf_infos,
                "getoutput_time",
                getattr(self.current_metrics.decode_perf_infos, "getoutput_time")
                + time_diff_ms,
            ),
            PERFTYPE.VISION_TOTAL_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.vision_perf_infos,
                "vision_total_time",
                getattr(self.current_metrics.vision_perf_infos, "vision_total_time")
                + time_diff_ms,
            ),
            PERFTYPE.VISION_PREPROCESS_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.vision_perf_infos,
                "vision_preprocess_time",
                getattr(
                    self.current_metrics.vision_perf_infos, "vision_preprocess_time"
                )
                + time_diff_ms,
            ),
            PERFTYPE.VISION_INPUT_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.vision_perf_infos,
                "setinput_time",
                getattr(self.current_metrics.vision_perf_infos, "setinput_time")
                + time_diff_ms,
            ),
            PERFTYPE.VISION_INFER_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.vision_perf_infos,
                "infer_time",
                getattr(self.current_metrics.vision_perf_infos, "infer_time")
                + time_diff_ms,
            ),
            PERFTYPE.VISION_OUTPUT_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.vision_perf_infos,
                "getoutput_time",
                getattr(self.current_metrics.vision_perf_infos, "getoutput_time")
                + time_diff_ms,
            ),
            PERFTYPE.AUDIO_TOTAL_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.audio_perf_infos,
                "audio_total_time",
                getattr(self.current_metrics.audio_perf_infos, "audio_total_time")
                + time_diff_ms,
            ),
            PERFTYPE.AUDIO_PREPROCESS_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.audio_perf_infos,
                "audio_preprocess_time",
                getattr(self.current_metrics.audio_perf_infos, "audio_preprocess_time")
                + time_diff_ms,
            ),
            PERFTYPE.AUDIO_INPUT_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.audio_perf_infos,
                "setinput_time",
                getattr(self.current_metrics.audio_perf_infos, "setinput_time")
                + time_diff_ms,
            ),
            PERFTYPE.AUDIO_INFER_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.audio_perf_infos,
                "infer_time",
                getattr(self.current_metrics.audio_perf_infos, "infer_time")
                + time_diff_ms,
            ),
            PERFTYPE.AUDIO_OUTPUT_TIME: lambda time_diff_ms: setattr(
                self.current_metrics.audio_perf_infos,
                "getoutput_time",
                getattr(self.current_metrics.audio_perf_infos, "getoutput_time")
                + time_diff_ms,
            ),
            PERFTYPE.PREFILL_LOAD_TIME: lambda time_diff_ms: setattr(
                self.current_metrics,
                "prefill_load_time",
                getattr(self.current_metrics, "prefill_load_time") + time_diff_ms,
            ),
            PERFTYPE.DECODE_LOAD_TIME: lambda time_diff_ms: setattr(
                self.current_metrics,
                "decode_load_time",
                getattr(self.current_metrics, "decode_load_time") + time_diff_ms,
            ),
            PERFTYPE.VISION_LOAD_TIME: lambda time_diff_ms: setattr(
                self.current_metrics,
                "vision_load_time",
                getattr(self.current_metrics, "vision_load_time") + time_diff_ms,
            ),
            PERFTYPE.AUDIO_LOAD_TIME: lambda time_diff_ms: setattr(
                self.current_metrics,
                "audio_load_time",
                getattr(self.current_metrics, "audio_load_time") + time_diff_ms,
            ),
        }
        ...
        self.get_start_time_mapping = {
            ...
            PERFTYPE.AUDIO_TOTAL_TIME: lambda: self.current_metrics.audio_perf_infos.total_time_start,
            PERFTYPE.AUDIO_PREPROCESS_TIME: lambda: self.current_metrics.audio_perf_infos.audio_preprocess_time_start,
            PERFTYPE.AUDIO_INPUT_TIME: lambda: self.current_metrics.audio_perf_infos.setinput_time_start,
            PERFTYPE.AUDIO_INFER_TIME: lambda: self.current_metrics.audio_perf_infos.infer_time_start,
            PERFTYPE.AUDIO_OUTPUT_TIME: lambda: self.current_metrics.audio_perf_infos.getoutput_time_start,
            PERFTYPE.AUDIO_LOAD_TIME: lambda: self.current_metrics.audio_load_time_start,
        }
    ...
    def calculate_prefill(self, metrics):
        if metrics.prefill_perf_infos.tokenizer_time > 0:
            metrics.prefill_perf_infos.tokenizer_speed = metrics.input_seq_length / (
                metrics.prefill_perf_infos.tokenizer_time / 1000
            )

        if metrics.prefill_perf_infos.embedding_time > 0:
            metrics.prefill_perf_infos.embedding_speed = metrics.input_seq_length / (
                metrics.prefill_perf_infos.embedding_time / 1000
            )

        if metrics.prefill_perf_infos.setinput_time > 0:
            metrics.prefill_perf_infos.setinput_speed = metrics.input_seq_length / (
                metrics.prefill_perf_infos.setinput_time / 1000
            )
        ...
        if metrics.prefill_perf_infos.infer_time > 0:
            metrics.prefill_perf_infos.infer_speed = (
                ...
                * metrics.input_seq_length
                / (metrics.prefill_perf_infos.infer_time / 1000)
            )

        if metrics.prefill_perf_infos.getoutput_time > 0:
            metrics.prefill_perf_infos.getoutput_speed = metrics.input_seq_length / (
                metrics.prefill_perf_infos.getoutput_time / 1000
            )
    ...
    def calculate_decode(self, metrics):
        ...
        if metrics.decode_perf_infos.infer_time > 0:
            metrics.decode_perf_infos.infer_speed = (
                metrics.output_seq_length * metrics.batch_size
            ) / (metrics.decode_perf_infos.infer_time / 1000)
        ...
        if metrics.decode_perf_infos.getoutput_time > 0:
            metrics.decode_perf_infos.getoutput_speed = (
                metrics.output_seq_length * metrics.batch_size
            ) / (metrics.decode_perf_infos.getoutput_time / 1000)
        ...
        if metrics.decode_perf_infos.total_time > 0:
            metrics.decode_perf_infos.total_speed = (
                metrics.output_seq_length * metrics.batch_size
            ) / (metrics.decode_perf_infos.total_time / 1000)
    ...
    def calculate_metrics(self):
        ...
        if (
            ...
            and metrics.num_images > 0
        ):
            metrics.vision_perf_infos.vision_preprocess_speed = metrics.num_images / (
                metrics.vision_perf_infos.vision_preprocess_time / 1000
            )
        if metrics.vision_perf_infos.infer_time > 0 and metrics.num_images > 0:
            metrics.vision_perf_infos.vision_infer_speed = metrics.num_images / (
                metrics.vision_perf_infos.infer_time / 1000
            )
        if metrics.vision_perf_infos.vision_total_time > 0 and metrics.num_images > 0:
            metrics.vision_perf_infos.vision_total_speed = metrics.num_images / (
        ...
        metrics.ttft = (
            ...
            + metrics.audio_perf_infos.audio_total_time
        )
        if metrics.output_seq_length > 0:
            metrics.tpot = metrics.decode_perf_infos.total_time / (
                metrics.output_seq_length * metrics.batch_size
            )  # TPOT = Decode total time / number of output tokens
    ...
    def show_summary(self) -> None:
        ...
        logger.success(
            f"  Input Length per Sample: {metrics.input_seq_length:>6} tokens"
        )
        logger.success(
            f"  Output Length per Sample: {(metrics.output_seq_length + 1):>6} tokens"
        )
        ...
        if metrics.prefill_load_time > 0:
            logger.success(
                f"  Prefill Model Load Time: {metrics.prefill_load_time:>7.2f}ms"
            )
        if metrics.decode_load_time > 0:
            logger.success(
                f"  Decode Model Load Time: {metrics.decode_load_time:>7.2f}ms"
            )
        if metrics.vision_load_time > 0:
            logger.success(
                f"  Vision Model Load Time: {metrics.vision_load_time:>7.2f}ms"
            )
        if metrics.audio_load_time > 0:
            logger.success(
                f"  Audio Model Load Time: {metrics.audio_load_time:>7.2f}ms"
            )
        ...
        if metrics.num_images > 0 and (
            ...
            or metrics.vision_perf_infos.vision_preprocess_time > 0
        ):
            logger.success(f"Vision Stage Performance:")
            logger.success(
                f"  Total Time: {metrics.vision_perf_infos.vision_total_time:>7.2f}ms | Speed: {metrics.vision_perf_infos.vision_total_speed:>7.2f} images/s"
            )
            logger.success(
                f"  Preprocessing Time: {metrics.vision_perf_infos.vision_preprocess_time:>5.2f}ms | Speed: {metrics.vision_perf_infos.vision_preprocess_speed:>7.2f} images/s"
            )
            logger.success(
                f"  API SetInput Time: {metrics.vision_perf_infos.setinput_time:>6.2f}ms"
            )
            logger.success(
                f"  API Inference Time: {metrics.vision_perf_infos.infer_time:>5.2f}ms | Speed: {metrics.vision_perf_infos.vision_infer_speed:>7.2f} images/s"
            )
            logger.success(
                f"  API GetOutput Time: {metrics.vision_perf_infos.getoutput_time:>5.2f}ms"
            )
        ...
        logger.success(
            f"  Total Time: {metrics.prefill_perf_infos.total_time:>7.2f}ms | Speed: {metrics.prefill_perf_infos.total_speed:>7.2f} tokens/s"
        )
        if metrics.prefill_perf_infos.tokenizer_time > 0:
            logger.success(
                f"  Tokenization Time: {metrics.prefill_perf_infos.tokenizer_time:>7.2f}ms"
            )
        else:
            logger.success(f"  Tokenization Time: Skipped (No operation)")
        logger.success(
            f"  Embedding Time: {metrics.prefill_perf_infos.embedding_time:>7.2f}ms"
        )
        logger.success(
            f"  API SetInput Time: {metrics.prefill_perf_infos.setinput_time:>6.2f}ms"
        )
        logger.success(
            f"  API Inference Time: {metrics.prefill_perf_infos.infer_time:>5.2f}ms | Prefill Speed: {metrics.prefill_perf_infos.infer_speed:>7.2f} tokens/s"
        )
        logger.success(
            f"  API GetOutput Time: {metrics.prefill_perf_infos.getoutput_time:>5.2f}ms"
        )

        # Decode Stage Performance
        logger.success(f"Decode Stage Performance:")
        logger.success(
            f"  Total Time: {metrics.decode_perf_infos.total_time:>7.2f}ms | Speed: {metrics.decode_perf_infos.total_speed:>7.2f} tokens/s"
        )
        if metrics.decode_perf_infos.tokenizer_time > 0:
            logger.success(
                f"  Tokenization Time: {metrics.decode_perf_infos.tokenizer_time:>7.2f}ms"
            )
        else:
            logger.success(f"  Tokenization Time: Skipped (No operation)")
        logger.success(
            f"  Embedding Time: {metrics.decode_perf_infos.embedding_time:>7.2f}ms"
        )
        logger.success(
            f"  API SetInput Time: {metrics.decode_perf_infos.setinput_time_per_token:>6.2f}ms"
        )
        logger.success(
            f"  API Inference Time: {metrics.decode_perf_infos.infer_time_per_token:>5.2f}ms | Decode Speed: {metrics.decode_perf_infos.infer_speed:>7.2f} tokens/s"
        )
        logger.success(
            f"  API GetOutput Time: {metrics.decode_perf_infos.getoutput_time_per_token:>5.2f}ms"
        )
