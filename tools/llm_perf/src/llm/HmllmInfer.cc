...
HmllmInfer::HmllmInfer(const std::string &prefillModelPath,
    ...
    this->prefill_length =
        prefill_module->GetInputInfo(prefill_module->GetInputName(0))
            .Shape()[1];
    ...
    auto get_context_length = [this](int input_idx) -> int64_t {
        return static_cast<int64_t>(
            prefill_module
                ->GetInputInfo(prefill_module->GetInputName(input_idx))
                .Shape()[2]);
    };
    ...
    if (attn_idx_start < prefill_module->GetInputNum()) {
        this->context_max_length = std::max(this->context_max_length,
                                            get_context_length(attn_idx_start));
    }
    this->batch =
        decode_module->GetInputInfo(decode_module->GetInputName(0)).Shape()[0];
    this->argmax_dim_len =
        decode_module->GetOutputInfo(decode_module->GetOutputName(0))
            .Shape()[2];
    ...
    for (int idx = 0; idx < this->prefill_module->GetInputNum(); idx++) {
        ...
        if (layer_name.find("model_layers") != std::string::npos) {
            ...
            CHECK_TCIM_RET_STATUS(
                decode_module->SetDevInput(layer_name, cache));
        ...
        if (layer_name.find("conv_cache") != std::string::npos) {
            ...
            auto cache = prefill_module->GetDevInput(layer_name);
            CHECK_TCIM_RET_STATUS(
                prefill_module->SetDevOutput(output_name, cache));
            CHECK_TCIM_RET_STATUS(
                decode_module->SetDevInput(layer_name, cache));
            CHECK_TCIM_RET_STATUS(
                decode_module->SetDevOutput(output_name, cache));
            auto input_info = prefill_module->GetInputInfo(layer_name);
            total_mem_size += input_info.MemSize();
        }
...
void HmllmInfer::prefill_input_init() {
    ...
    for (int idx = 0; idx < attn_idx_start; ++idx) {
        ...
        auto input_info =
            prefill_module->GetInputInfo(input_name).AsContiguous();
    ...
    for (int idx = attn_idx_start; idx < prefill_module->GetInputNum(); idx++) {
        ...
        if (input_name.find("conv_cache") != std::string::npos ||
            ...
            tcim::Tensor input_tensor =
                tcim::Tensor::CreateHostTensor(input_info);
            prefill_input_map.insert(
                std::pair<std::string, tcim::Tensor>(input_name, input_tensor));
...
void HmllmInfer::decode_input_init() {
    ...
    for (int idx = 0; idx < attn_idx_start; ++idx) {
        ...
        auto input_info =
            decode_module->GetInputInfo(input_name).AsContiguous();
        tcim::Tensor input_tensor = tcim::Tensor::CreateHostTensor(input_info);
...
void HmllmInfer::DebugModelInfo(tcim::Module &module,
    ...
    for (int idx = 0; idx < output_num; idx++) {
        ...
        std::cout << "Output[" << output_name << "] " << output_info
                  << std::endl;
    }
}
...
void HmllmInfer::PrefillSetInputDatas(void *data, int32_t valid_length,
    ...
    for (int idx = 0; idx < attn_idx_start; idx++) {
        ...
        if (input_name.find("input_1") != std::string::npos) {
            ...
            for (int i = valid_length; i < valid_length + this->prefill_length;
                 ++i) {
            ...
            std::vector<float16> attn_mask;
            for (int i = 0; i < this->prefill_length; ++i) {
                if (i < current_length) {
                    attn_mask.emplace_back(static_cast<float16>(1.0f));
                } else {
...
void HmllmInfer::PrefillGetOutputDatas(std::vector<int32_t> &ids) {
    ...
    auto output_info =
        prefill_module->GetOutputInfo(output_name).AsContiguous();
...
void HmllmInfer::DecodeSetInputDatas(void *data, int32_t context_length) {
    for (int idx = 0; idx < attn_idx_start; idx++) {
        ...
        if (input_name.find("input_1") != std::string::npos) {
            ...
            CHECK_TCIM_RET_STATUS(
                tensor.Buffer().CopyFromHost(&mask_value, memSize));
        } else if (input_name.find("attention_mask") != std::string::npos) {
            CHECK_TCIM_RET_STATUS(tensor.Buffer().CopyFromHost(
...
void HmllmInfer::perf_llm(const uint32_t input_tokens_len,
    ...
    for (int round = 0; round < prefill_loop_round; round++) {
        ...
        std::cout << '\r' << "Prefill: " << std::setw(3)
                  << int(prefill_ratio * 100) << "% |"
                  << std::string(filled, '*')
