...
class YUVFormat:
    ...
    def __call__(self, img: torch.Tensor):
        # b c h w
        batch, _, img_h, img_w = img.size()
        # breakpoint()
        y, u, v = torch.split(img, 1, dim=1)

        if self.fmt == "rand":
            self.fmt = random.choice(["420", "444", "422"])

        if self.fmt in ["444", "YUV444"]:
            uv = torch.stack([u, v], dim=-1)
            y = y.view(batch, -1)
            uv = uv.view(batch, -1)
            yuv = torch.cat((y, uv), 1)
            return yuv.view((batch, 3, img_h, img_w))

        div_w, div_h = self._MAP[self.fmt]
        if self.interpolation:
            u = F.interpolate(
                u,
                size=(img_h // div_h, img_w // div_w),
                mode="bilinear",
                align_corners=False,
            )
            v = F.interpolate(
        ...
        uv = uv.view(batch, -1)
        yuv = torch.cat((y, uv), 1)
        result = torch.zeros(batch, img_h * img_w * 3).to(yuv.device)
        result[..., : yuv.shape[-1]] = yuv
        return result.view((batch, 3, img_h, img_w))


class QRawToYuv(nn.Module):
    def __init__(
        self,
        input_color_type="RGB",
        toYUV_format="YUV422",
        version="BT601",
        interpolation=True,
    ) -> None:
        super().__init__()
        self.input_color_type = input_color_type
        if self.input_color_type in ["RGB"]:
            Mb = M_RGB2YUV[version]
        else:
            Mb = M_BGR2YUV[version]

        self.register_buffer("M", torch.Tensor(Mb[0:3]).T)
        self.register_buffer("b", torch.Tensor(Mb[3]).view(1, 3, 1, 1))

        self.toYUV_format = toYUV_format
        self.interpolation = interpolation
        self.version = version
        self.op_class = "ToYuv"

    def forward(self, img: torch.Tensor):
        result = torch.einsum("ij,bjhw->bihw", [self.M, img.float()])  # BGR2YUV
        result = result + self.b
        result.clip_(0, 255).round_()

        formatter = YUVFormat(self.toYUV_format, self.interpolation)
...
class Qwen3VL:
    ...
    def load_and_process_image(self, image: Any):
        """
    ...
    def load_and_process_image_v2(self, image: Any):
        """
        Loads an image from the given path, converts to RGB, resizes proportionally if needed,
        and pads to (self.image_size_w, self.image_size_h) with (114,114,114) background.
        Returns the processed PIL image.
    ...
    def chat_vit_prefill(self, prompt: str, image: str = None):
        self.generated_ids = []
        self.skip_tokens = 0
        self.slide_len = 10
        if image is not None:
            ...
            if self.resize_v1:
                pil_image = self.load_and_process_image(image)
            else:
                pil_image = self.load_and_process_image_v2(image)
        else:
            pil_image = None
