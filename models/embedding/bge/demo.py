...
class HmBGEReRanker(nn.Module):
    ...
    def get_token_types_ids(self, config_file):
        if not os.path.exists(config_file):
            logger.error(f"{config_file} is not exists, don not get token_type_ids default!")
            assert(0)
    ...
    def get_detailed_inputs(self, sentence_pairs: Union[str, List[str]]):
        ...
        if self.query_instruction_for_rerank is not None:
            if self.passage_instruction_for_rerank is None:
                return [
                    [
                        get_detailed_instruct(self.query_instruction_format, self.query_instruction_for_rerank, sentence_pair[0]),
                        sentence_pair[1]
                ...
                return [
                    [
                        get_detailed_instruct(self.query_instruction_format, self.query_instruction_for_rerank, sentence_pair[0]),
                        get_detailed_instruct(self.passage_instruction_format, self.passage_instruction_for_rerank, sentence_pair[1])
                    ] for sentence_pair in sentence_pairs
                ]
        else:
            if self.passage_instruction_for_rerank is None:
    ...
    def reranker(self, sentences_pairs: Union[List[Tuple[str, str]], Tuple[str, str]], **kwargs: Any) -> np.ndarray:
        self.passage_instruction_for_rerank = kwargs.get("passage_instruction", None)
        self.passage_instruction_format = kwargs.get("passage_instruction_format", "{}{}")
        ...
        for start_index in range(0, len(sentences_pairs), self.batch_size):
            sentences_batch = sentences_pairs[start_index:start_index+self.batch_size]
            queries = [s[0] for s in sentences_batch]
            passages = [s[1] for s in sentences_batch]
            queries_inputs_batch = self.tokenizer(queries, 
                                                  return_tensors=None, 
                                                  add_special_tokens=False, 
                                                  max_length=self.query_max_length, 
                                                  truncation=True)["input_ids"]
            passages_inputs_batch = self.tokenizer(passages,
                                                   return_tensors=None,
                                                   add_special_tokens=False,
                                                   max_length=self.max_length,
                                                   truncation=True)["input_ids"]
            for q_inp, d_inp in zip(queries_inputs_batch, passages_inputs_batch):
                item = self.tokenizer.prepare_for_model(q_inp,
                                                        d_inp,
        ...
        for start_index in range(0, len(all_inputs_sorted), self.batch_size):
            ...
            inputs = self.tokenizer.pad(sentences_batch,
                                        padding=True,
                                        return_tensors='pt')
            ...
            if "token_type_ids" not in inputs:
                if hasattr(self, "token_type_ids"):
                    ...
                    buffered_token_type_ids_expanded = buffered_token_type_ids.expand(cur_batch, cur_seq_len)
                    inputs["token_type_ids"] = buffered_token_type_ids_expanded
                else:
                    inputs["token_type_ids"] = torch.zeros(inputs["input_ids"].shape, dtype=torch.long, device="cpu")
            input_dict = {} 
            for input_name in self.input_infos.keys():
                if cur_batch < self.batch_size or cur_seq_len < self.max_length:
                    input_data = torch.zeros(self.input_infos[input_name].shape, dtype=torch.long)
                    input_data[:cur_batch, :cur_seq_len] = inputs[input_name]
                else:
                    input_data = inputs[input_name]
                input_data = input_data.numpy().astype(self.input_infos[input_name].dtype)
            ...
            if self.model_type == "houmo":
                ...
                self.engine.run()
                self.engine.sync()
                cur_scores = self.engine.get_output(self.output_name).numpy()[:cur_batch, ...]
            elif self.model_type == "onnx":
...
class HmBGEM3(nn.Module):
    ...
    def pooling(self, last_hidden_state: torch.Tensor, attention_mask: Optional[torch.Tensor] = None):
    ...
    def embedder(self, sentences, **kwargs: Any) -> np.ndarray:
        ...
        if instruction is not None:
            if isinstance(sentences, str):
                sentences = get_detailed_instruct(instruction_format, instruction, sentences)
            else:
                sentences = [get_detailed_instruct(instruction_format, instruction, sentence) for sentence in
                             sentences]
        ...
        all_embeddings = []
        for start_index in range(0, len(sentences), self.batch_size):
            inputs_batch = all_inputs_sorted[start_index:start_index+self.batch_size]
            ...
            if "token_type_ids" not in inputs_batch:
                if hasattr(self, "token_type_ids"):
                    ...
                    inputs_batch["token_type_ids"] = torch.zeros(inputs_batch["input_ids"].shape, dtype=torch.long, device="cpu")
            input_dict = {}
            for input_name in self.input_infos.keys():
                if cur_batch < self.batch_size or cur_seq_len < self.max_length:
                    input_data = torch.zeros(self.input_infos[input_name].shape, dtype=torch.long)
...
def xh2_demo(args):
    ...
    if args.mode == "reranker" or args.mode == "all":
        ...
        logger.info("Time taken to reorder the corpus and query statements: %.6fms"%(reranker_time * 1000))
