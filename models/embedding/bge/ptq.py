...
def get_net_input_output_infos(model_path):
    ...
    for idx, net_input in enumerate(onnx_model.graph.input):
        ...
        input_shape = [
            ...
            for d in net_input.type.tensor_type.shape.dim
        ]
        input_info = {
            "input_shape": input_shape,
            "dtype": onnx.mapping.TENSOR_TYPE_TO_NP_TYPE.get(
                net_input.type.tensor_type.elem_type
            ).name,
        }
    ...
    for idx, net_output in enumerate(onnx_model.graph.output):
        ...
        output_info = {
            "dtype": onnx.mapping.TENSOR_TYPE_TO_NP_TYPE.get(
                net_output.type.tensor_type.elem_type
            ).name
        }
...
def quantize(args, model_path, model_name):
    ...
    for idx, data in enumerate(in_datas):
        data = (
            ...
            .cpu()
            .numpy()
            .astype(quant_input_infos[list(quant_input_infos.keys())[idx]]["dtype"])
        )
...
def main():
    ...
    onnx_path = os.path.join(
        args.model_path, f"{bge_m3_model_name}/{bge_m3_model_name}.onnx"
    )
    ...
    onnx_path = os.path.join(
        args.model_path, f"{bge_reranker_model_name}/{bge_reranker_model_name}.onnx"
    )
    if not os.path.exists(onnx_path):
        logging.error(f"{onnx_path} is not found!")
        return
