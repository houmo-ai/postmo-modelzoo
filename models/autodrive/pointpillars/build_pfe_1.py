...
def build(args):
    ...
    if stage == 'test' or stage == 'all':
        ...
        for input in inputs:
            ...
            input_data_crop = np.zeros((1, 12032, 20, 64)).astype(np.int8)
            input_data_crop[:,0:1984,:,:] = input_data
            module.set_input(input.name, input_data_crop, image_format)
