...
def build(args=None):
    ...
    if stage == 'build' or stage == 'all':
        ...
        input_cfg = {}
        input_cfg['weight'] = tcim.HMInput(shape=[6, 256, 4, 900])
        input_cfg['mask'] = tcim.HMInput(shape=[6, 4, 1, 900], layout='NCHW')
        compile_config = {
        ...
        tcim.build.build_from_hmonnx(onnx_model, model_name=model_name, batch_size=None, inputs=input_cfg, compiler_cfg=compile_config)
    ...
    if stage == 'test' or stage == 'all':
        ...
        for id in range(input_num):
            ...
            if input_name == 'mask':
                input_data = np.concatenate([input_data for i in range(4)], axis=1)
                print("input[{}] concat to {}".format(input_name, input_data.shape))
            elif input_name == 'weight':
                input_data = np.broadcast_to(input_data, (6, 256, 4, 900))
                input_data = np.transpose(input_data, [0, 2, 3, 1])
                print("input[{}] broadcast and transpose to {}".format(input_name, input_data.shape))
            else:
                input_data = np.transpose(input_data, [0, 2, 3, 1])
                print("input[{}] transpose to {}".format(input_name, input_data.shape))
