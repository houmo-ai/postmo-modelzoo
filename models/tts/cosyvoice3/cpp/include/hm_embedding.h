...
class HmEmbedding {
 public:
   ...
   */
  TensorType *EmbeddingTokens(const std::vector<int> &ids);
  int get_vocab_size() const { return vocab_size; }

  TensorType *get_embedding_ptr() const { return embed_w.get(); }

 private:
  std::unique_ptr<TensorType[]> embed_w;
  std::unique_ptr<TensorType[]> ptr;
