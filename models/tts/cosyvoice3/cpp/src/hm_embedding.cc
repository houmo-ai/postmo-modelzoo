...
HmEmbedding::HmEmbedding(const std::string& embeddingWeightPath,
  ...
  try {
    int n_bytes = 0;
    embed_w = readEmbeddingWeight<TensorType>(
        embeddingWeightPath, n_bytes, prefill_length * embedding_length);
    vocab_size = n_bytes / (sizeof(TensorType) * embedding_length);
    std::cout << "[INFO] vocab_size: " << vocab_size
              << ", embedding_length: " << embedding_length
              << ", prefill_length: " << prefill_length << "\n";
    if (vocab_size <= 0 ||
        (vocab_size * embedding_length * sizeof(TensorType) > n_bytes)) {
      throw std::runtime_error(
          "Invalid embedding weight file: vocab size must be positive and file "
          "size mismatch.");
    }

    if (vocab_size * embedding_length * sizeof(TensorType) < n_bytes) {
  ...
  ptr = std::make_unique<TensorType[]>(prefill_length * embedding_length);
}
...
TensorType* HmEmbedding::EmbeddingTokens(const std::vector<int>& ids) {
  if (ids.empty()) {
    return nullptr;
  }
