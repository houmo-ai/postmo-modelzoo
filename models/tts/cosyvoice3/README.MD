# Fun-CosyVoice3-0.5B-2512

本示例展示如何将 **Fun-CosyVoice3-0.5B-2512** 模型进行量化与编译，并在后摩 **XH2** 设备上进行推理演示。

[TOC]

## 1 模型说明

本例使用的模型实现来源于 ModelScope 的预训练模型：

- 模型名称: Fun-CosyVoice3-0.5B-2512
- 来源: https://www.modelscope.cn/models/FunAudioLLM/Fun-CosyVoice3-0.5B-2512
- 许可: Apache License 2.0 (https://www.apache.org/licenses/LICENSE-2.0)

预训练模型在运行时下载，工程发布 **不包含** 该模型文件。

本例只适用于xh2。

`cosyvoice3` 目录包含以下流程脚本：

- `get_model.py`: 下载原始模型文件（`raw`）或已编译模型（`hmm`）
- `ptq.py`: PTQ 量化（生成 HMONNX/权重/embedding 等产物）
- `build.py`: 使用 tcim 将 HMONNX 编译为 `.hmm`
- `demo.py`: 在 XH2 上执行端到端 cosyvoice3 推理示例
- `test.sh`: 一键下载/量化/编译/演示

默认参数统一由 `config.yaml` 提供，当前默认模型为：

- `model_name=cosyvoice3`
- `model_size=0.5b-2512`
- `context_length=2k`
- `input_sequence_length=256`
- `ndevice=1`

## 2 快速开始

说明：

- **量化需要 GPU 环境**

### 2.1 环境准备

安装示例运行所需的python依赖，建议使用venv

```bash
pip3 install -r requirements.txt
```

`test.sh` 在检测到当前目录存在 `requirements.txt` 时，会自动创建 `cosyvoice3_venv` 虚拟环境并安装依赖。

其中针对 `aarch64` 平台，脚本额外做了如下适配：

- 执行 `pip3 install scikit-learn==1.3.0 --no-binary scikit-learn -i https://pypi.tuna.tsinghua.edu.cn/simple`
- 补充虚拟环境下的 `site-packages` 到 `PYTHONPATH`
- 设置 `LD_PRELOAD=/usr/lib/aarch64-linux-gnu/libgomp.so.1`
- 设置 `OMP_NUM_THREADS=1`
- 设置 `MKL_NUM_THREADS=1`

如需手动执行与 `test.sh` 一致的 `aarch64` 环境配置，可参考：

```bash
pip3 install scikit-learn==1.3.0 --no-binary scikit-learn -i https://pypi.tuna.tsinghua.edu.cn/simple
export PYTHONPATH="./cosyvoice3_venv/lib/python3.9/site-packages:$PYTHONPATH"
export LD_PRELOAD=/usr/lib/aarch64-linux-gnu/libgomp.so.1
export OMP_NUM_THREADS=1
export MKL_NUM_THREADS=1
```

### 2.2 获取模型

下载 **已编译模型**（推荐，仅跑演示）：

```bash
cd models/tts/cosyvoice3
python3 get_model.py --type hmm --model_name cosyvoice3 --model_size 0.5b-2512
```

下载 **原始模型文件**（用于自行量化/编译）：

```bash
cd models/tts/cosyvoice3
python3 get_model.py --type raw --model_name cosyvoice3 --model_size 0.5b-2512
```

常用参数：

```bash
python3 get_model.py -h
```

### 2.3 量化

量化需要使用GPU，并在宿主机安装与CUDA12.8兼容的CUDA和驱动。启动docker时将GPU映射到docker内，并指定可用的GPU ID。参考命令：

```bash
docker run -it --gpus all --pid=host -w /hmdd -v $PWD:/hmdd --shm-size 64g harbor.houmo.ai/toolchain/release:Dadao-xh2-x.y.z-ubuntu24.04-x86.64 /bin/bash
```

进入 docker 后 source 环境变量，并进入 `models/tts/cosyvoice3` 目录，执行 `ptq.py` 脚本：

```bash
source env.sh
cd models/tts/cosyvoice3
python3 ptq.py --model_name cosyvoice3 --model_size 0.5b-2512
```

常用参数：

- `--model_dir`: 原始模型目录，默认从 `config.yaml` 的 `modelscope_repo` 推导为 `./Fun-CosyVoice3-0.5B-2512`
- `--model_name`: 输出模型名前缀，默认 `cosyvoice3`
- `--model_size`: 输出模型规格，默认 `0.5b-2512`
- `--context_length`: 最大序列长度，默认从 `config.yaml` 读取为 `2k`
- `--input_sequence_length`: 输入序列长度，默认 `256`
- `--quant_type`: 量化类型，默认 `w8a16h1_sefp`

默认产物目录为 `output/$HOUMO_TARGET/hmquant`，其中会包含多子目录与文件（示例）：

```text
output/xh2/hmquant
|-- campplus/
|-- speech_tokenizer/
|-- llm_prefill/
|-- llm_decode/
|-- llm_decoder/
|-- flow_encoder/
|-- flow_spk/
|-- flow_decoder/
|-- hift/
`-- quant_embedding.pt
`-- flow_input_embedding.pt
`-- llm_sos_embedding.pt
`-- llm_task_id_embedding.pt
`-- llm_speech_embedding.pt
```

### 2.4 编译（HM-ONNX → HMM）

将量化产物编译为芯片可运行的 `.hmm`：

```bash
cd models/tts/cosyvoice3
python3 build.py --model_name cosyvoice3 --model_size 0.5b-2512
```

编译产物默认位于 `output/$HOUMO_TARGET/` 目录下，包含多个 `.hmm` 文件。

默认输出名称分别为：

- `cosyvoice3-0.5b-2512_campplus.hmm`
- `cosyvoice3-0.5b-2512_speech_tokenizer.hmm`
- `cosyvoice3-0.5b-2512_llm_qwen2_prefill.hmm`
- `cosyvoice3-0.5b-2512_llm_qwen2_decode.hmm`
- `cosyvoice3-0.5b-2512_llm_decoder.hmm`
- `cosyvoice3-0.5b-2512_flow_encoder.hmm`
- `cosyvoice3-0.5b-2512_flow_spk.hmm`
- `cosyvoice3-0.5b-2512_flow_decoder.hmm`
- `cosyvoice3-0.5b-2512_hift_part1.hmm`
- `cosyvoice3-0.5b-2512_hift_part2.hmm`

说明：本仓库的 `build.py` 内部包含多个子模块的编译入口；如仅跑演示，优先使用 `get_model.py --type hmm` 获取已编译模型。

### 2.5 演示（cosyvoice3 推理）

执行推理示例（会下载 tokenizer 等必要资源，结果音频写入 `--output_dir` 目录， 默认为 `./results`）：

```bash
cd models/tts/cosyvoice3
python3 demo.py --model_name cosyvoice3 --model_size 0.5b-2512
```

demo 默认会从 `config.yaml` 和 `${model_name}-${model_size}` 推导 tokenizer 与各子模型路径。

更多参数：

```bash
python3 demo.py -h
```

**提供c++示例，详细介绍参考cpp/README.MD。**

## 3 一键评估

以上步骤可以通过 test.sh 脚本执行。默认执行 demo 步骤：

```bash
# 默认执行 demo，会先下载预编译模型
bash test.sh

# 完整流程：下载原始模型 + 量化 + 编译 + 演示
bash test.sh -s all
```

当前脚本内部固定使用以下默认值：

- `MODEL_NAME=cosyvoice3`
- `MODEL_SIZE=0.5b-2512`

对应执行行为如下：

- `quant`：如未传 `--skip_download`，先下载 raw 模型，再执行 `python3 ptq.py --model_name "${MODEL_NAME}" --model_size "${MODEL_SIZE}"`
- `build`：执行 `python3 build.py --model_name "${MODEL_NAME}" --model_size "${MODEL_SIZE}"`
- `demo`：如未传 `--skip_download`，先下载预编译模型，再执行 `python3 demo.py --model_name "${MODEL_NAME}" --model_size "${MODEL_SIZE}"`
- `demo`：随后会继续构建 `cpp/` 示例、转换 embedding，并执行 `./bin/cosyvoice3-demo`

test.sh 支持以下参数：

```bash
Usage: test.sh [options]
Options:
  -s, --step              Step to run. Default: demo. Choices: demo, build, quant, all.
                          Supports comma-separated values or repeated flags, e.g. -s quant,build or -s quant -s build.
  -size, --model_size     Model size.
  -name, --model_name     Model name.
  --skip_download         Skip model download steps.
  -h, --help              Show this help message.
```

示例：

```bash
# 仅运行 quant，会先下载 raw 模型并使用脚本内默认的 cosyvoice3-0.5b-2512 参数量化
bash test.sh -s quant

# 仅运行 build，要求当前目录下已经有量化输出
bash test.sh -s build

# 仅运行 demo，会先下载预编译模型，然后继续跑 python + cpp 两套演示
bash test.sh -s demo

# 完整流程（下载原始模型、量化、编译、演示）
bash test.sh -s all

# 跳过模型下载，使用当前目录下已有的模型文件执行 demo
bash test.sh -s demo --skip_download
```

## 4 参考结果

### 4.1 演示结果

```bash
Start inference zero shot, zero_shot_spk_id: , speed: 1.0, text_frontend: True
  0%|                       | 0/1 [00:00<?, ?it/s]
synthesis text 下面为您朗诵一段绕口令，希望您[j][ǐ]予好评，朗诵开始:八百标兵奔北坡，北坡炮兵并排跑，炮兵怕把标兵碰，标兵怕碰炮兵炮。
tts, uuid: 617dc7a9-1ec2-11f1-9ab6-4dff60635bb1, release resources.
generate tts speech successfully.
100%|████████████████████████| 1/1 [00:34<00:00, 34.02s/it]

  0%|                        | 0/1 [00:00<?, ?it/s]
synthesis text You are a helpful assistant.<|endofprompt|>[breath]因为他们那一辈人[breath]在乡里面住的要习惯一点，[breath]邻居都很活络，[breath]嗯，都很熟悉。[breath]
tts, uuid: 75c30f6b-1ec2-11f1-b204-4dff60635bb1, release resources.
generate tts speech successfully.
100%|████████████████████████| 1/1 [00:32<00:00, 32.38s/it]

  0%|                        | 0/1 [00:00<?, ?it/s]
synthesis text 好少咯，一般系放嗰啲国庆啊，中秋嗰啲可能会咯。
tts, uuid: 890d3c59-1ec2-11f1-abc6-4dff60635bb1, release resources.
generate tts speech successfully.
100%|████████████████████████| 1/1 [00:31<00:00, 31.47s/it]
```

### 4.2 性能结果

#### 性能评估指标

性能指标说明（每条报告对应一次 `tts()`）：

一次 `tts()`：指对 **一个待合成的文本分段（utterance）** 发起一次完整端到端合成调用，包含 **LLM 生成 speech token** 与 **`token2wav()` 生成波形** 两阶段。长文本会先被 `text_normalize(..., split=True)` 切分为多个分段，因此可能对应多次 `tts()` 与多条性能报告。

- **LLM Total Cost**：LLM 阶段总耗时（ms），对应 speech token 生成阶段累计时间。
- **LLM Prefill Speed**：LLM prefill 阶段速度（tokens/s）。token 数按 LLM 的输入序列长度统计。
- **TTFT (Time to First Token)**：从进入 LLM 推理到生成首个 speech token 的时间（ms）。
- **TPOT (Time Per Output Token)**：LLM decode 阶段生成 token 的速度（tokens/s）。
- **TTS Total Cost**：`token2wav()` 阶段耗时（ms），不包含 LLM 阶段。
- **TTS Real-Time Factor (RTF)**：当前计算方式为 `RTF = E2E Latency / 音频时长`。
- **TTS Generate Speed**：相对实时倍速，等价于 `1 / RTF`（例如 `0.42 x real-time`）。
- **E2E Latency (End-to-End Latency)**：端到端时延（秒），用于表示一次 `tts()` 调用从开始执行到返回最终音频的整体耗时。

#### M50 2core
```bash
[Perf #0]
LLM Total Cost 4830.223 ms
LLM Prefill Speed: 2998.75 tokens/s
TTFT (Time to First Token): 63.516 ms
TPOT (Time Per Output Token): 85.59 tokens/s
TTS Total Cost: 29395.319 ms
TTS Real-Time Factor(RTF): 2.222735
TTS Generate Speed: 0.45 x real-time
E2E Latency (End-to-End Latency): 34.230 s

[Perf #1]
LLM Total Cost 2773.767 ms
LLM Prefill Speed: 884.67 tokens/s
TTFT (Time to First Token): 58.831 ms
TPOT (Time Per Output Token): 91.72 tokens/s
TTS Total Cost: 29388.707 ms
TTS Real-Time Factor(RTF): 3.216384
TTS Generate Speed: 0.31 x real-time
E2E Latency (End-to-End Latency): 32.164 s

[Perf #2]
LLM Total Cost 1599.579 ms
LLM Prefill Speed: 838.54 tokens/s
TTFT (Time to First Token): 59.643 ms
TPOT (Time Per Output Token): 85.07 tokens/s
TTS Total Cost: 29399.501 ms
TTS Real-Time Factor(RTF): 5.871297
TTS Generate Speed: 0.17 x real-time
E2E Latency (End-to-End Latency): 31.000 s
```

## 5 免责声明

您明确了解并同意，以下链接中的软件、数据或者模型由第三方提供并负责维护。在以下链接中出现的任何第三方的名称、商标、标识、产品或服务并不构成明示或暗示与该第三方或其软件、数据或模型的相关背书、担保或推荐行为。您进一步了解并同意，使用任何第三方软件、数据或者模型，包括您提供的任何信息或个人数据（不论是有意或无意地），应受相关使用条款、许可协议、隐私政策或其他此类协议的约束。因此，使用链接中的软件、数据或者模型可能导致的所有风险将由您自行承担。
