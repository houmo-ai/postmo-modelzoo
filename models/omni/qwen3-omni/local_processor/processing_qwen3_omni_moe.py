...
#
# File: processing_qwen3_omni_moe.py
# Description:
#   Qwen3-Omni processor adapted for the xh2 model zoo (xh2modelzoo).
...
class Qwen3OmniMoeProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        # Pin the video frames to the same fixed square resolution as images, so
        # each frame yields a constant token count matching the statically
        # exported HMONNX vision encoder (448x448 -> 196 tokens per frame).
        self._sync_video_resolution()

    def _sync_video_resolution(self):
        """Mirror the image processor's fixed square resolution onto the video
        processor so video frames yield the same constant per-frame token count."""
        fixed_size = getattr(self.image_processor, "fixed_vision_size", None)
        if fixed_size is not None and self.video_processor is not None:
            fixed_pixels = fixed_size * fixed_size
            self.video_processor.min_pixels = fixed_pixels
            self.video_processor.max_pixels = fixed_pixels
            if hasattr(self.video_processor, "size") and isinstance(self.video_processor.size, dict):
                self.video_processor.size = {"shortest_edge": fixed_pixels, "longest_edge": fixed_pixels}

    def set_vision_resolution(self, fixed_size):
        """Inject the vision encoder's fixed square resolution at runtime so the
        processor auto-aligns its image/video token counts to the exported model.

        Pass the side length in pixels (e.g. 448 for a 448x448 / 196-token graph),
        or None to restore the original dynamic smart_resize. Typically called
        right after loading the HMONNX vision graph, deriving `fixed_size` from
        the graph's input shape so a re-export at a new resolution needs no manual
        edit here."""
        self.image_processor.set_fixed_vision_size(fixed_size)
        self._sync_video_resolution()
