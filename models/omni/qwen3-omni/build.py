...
# File: build.py
# Description:
#   qwen3-omni model compilation and consistency comparison.
#
...
def test(model_name, model_dir, output_dir, profile, batch=1, prefix=None, ndevice=1):
    import tcim_lite
    ...
    start = time.time()
    if ndevice == 2 and model_name in ["qwen3-omni_prefill", "qwen3-omni_decode"]:
        llm_dev_manager = tcim_lite.runtime.DevManager([0, 1], "Xh2HalBackend")
        option = tcim_lite.runtime.Option(llm_dev_manager)
        model_path = os.path.join(output_dir, f"{model_name}.hmms")
        print(model_path)
        module = tcim_lite.runtime.load(model_path, option=option)
    else:
        print(model_path)
