...
TALKER_SUPPRESS_TOKENS = [i for i in range(TALKER_VOCAB_SIZE - 1024, TALKER_VOCAB_SIZE)
...
def _format_perf_report(perf: Dict[str, float]) -> str:
    ...
    L.append("=" * 68)
    L.append(f"  Input Length: {int(perf.get('input_tokens', 0)):>6} tokens"
             f"  (audio={int(perf.get('audio_tokens', 0))}, image={int(perf.get('image_tokens', 0))})")
    L.append(f"  Output Length: {int(perf.get('thinker_decode_tokens', 0)) + 1:>6} text tokens")
    L.append("-" * 68)
    L.append("Encoders:")
    ...
    if perf.get("thinker_prefill_s"):
        L.append(f"  Prefill: {_fmt_ms(perf['thinker_prefill_s'])} | "
    ...
    if perf.get("thinker_decode_s") and dt > 0:
        L.append(f"  Decode: {_fmt_ms(perf['thinker_decode_s'])} | "
    ...
    if self_has(perf, "talker_s"):
        ...
        L.append(f"  Talker Generate: {_fmt_ms(perf['talker_s'])} | "
        ...
        if audio_s > 0:
            speech_s = perf.get("speech_total_s", perf.get("talker_s", 0) + perf.get("code2wav_s", 0))
            rtf = speech_s / audio_s if audio_s > 0 else 0.0
            L.append(f"  Generated Audio: {_fmt_s(audio_s)} | RTF: {rtf:.4f} "
                     f"({(1.0 / rtf):.2f}x real-time)" if rtf > 0 else "")
    L.append("-" * 68)
...
def load_codec_embeddings(hmquant_dir: Path) -> List[torch.Tensor]:
    ...
    return [e.to(torch.float16) if isinstance(e, torch.Tensor)
            else e.weight.data.to(torch.float16) for e in codec_list]


def run_projection_engine(engine, x: torch.Tensor, static: int = TALKER_STATIC_SEQ) -> torch.Tensor:
...
class ThinkerSamplingManager:
    ...
    def __init__(self, do_sample: bool = False, temperature: float = 1.0,
                 top_k: int = 50, top_p: float = 1.0,
                 repetition_penalty: float = 1.0, min_tokens_to_keep: int = 1):
    ...
    def _apply_repetition_penalty(self, logits: np.ndarray, previous_tokens: Optional[List[int]] = None) -> np.ndarray:
    ...
    def _process_logits(self, logits: np.ndarray, previous_tokens: Optional[List[int]] = None) -> np.ndarray:
    ...
    def sample(self, logits_np: np.ndarray, generated_ids: Optional[List[int]] = None) -> int:
...
def greedy_or_sample(logits_np: np.ndarray, do_sample: bool,
                     temperature: float = 1.0, top_k: int = 50,
                     repetition_penalty: float = 1.0,
                     generated_ids: list = None,
                     suppress_tokens: Optional[List[int]] = None) -> int:
...
# ═══════════════════════════════════════════════════════════════════════════════
_MD_LINK_RE = re.compile(r'!?\[([^\]]*)\]\([^)]*\)')  # [text](url) / ![alt](src) -> text
...
def split_text_into_segments(text: str, max_chars: int = 60,
...
class Qwen3OmniHmmPipeline:
    ...
    def __init__(self, args):
        ...
        self.vision_start_token_id = int(self.tokenizer.convert_tokens_to_ids(self.processor.vision_bos_token))
        self.vision_end_token_id = int(self.tokenizer.convert_tokens_to_ids(self.processor.vision_eos_token))
        self.audio_start_token_id = int(self.tokenizer.convert_tokens_to_ids(self.processor.audio_bos_token))
        self.audio_end_token_id = int(self.tokenizer.convert_tokens_to_ids(self.processor.audio_eos_token))
        ...
        logger.info(f"  codec_embeddings: {len(self.codec_embeddings)}x{self.codec_embeddings[0].shape}")
    ...
    def _align_vision_resolution(self):
        ...
        try:
            ...
            if vis_h == vis_w and vis_h > 0:
                ...
                logger.info(f"  Vision resolution aligned to exported graph: {vis_h}x{vis_w}")
            else:
                logger.warning(f"  Visual graph input {self.visual_shape} is non-square; keeping processor default.")
        except Exception as e:
            logger.warning(f"  Could not auto-align vision resolution from graph ({e}); keeping processor default.")
    ...
    def _load_engines(self):
        ...
        if ndevice == 1:
            if args.run_device_num == 1:
                ...
                raise ValueError(f"Unsupported run_device_num={args.run_device_num} for ndevice=1")
        elif ndevice == 2:
        ...
        self.visual_engine = self._load_engine(
            args.visual_path, self.mm_wm, "visual")
        self.audio_engine = self._load_engine(
            args.audio_path, self.mm_wm, "audio")
        self.prefill_engine = self._load_engine(
            _llm_path(args.prefill_path), self.llm_wm, "prefill")
        self.decode_engine = self._load_engine(
            _llm_path(args.decode_path), self.llm_wm, "decode",
            use_dummy_cache=True, num_layers=THINKER_NUM_LAYERS)

        if self.enable_audio_generation:
            self.talker_prefill_engine = self._load_engine(
                args.talker_prefill_path, self.talker_wm,
                "talker_prefill")
            self.talker_decode_engine = self._load_engine(
                args.talker_decode_path, self.talker_wm,
                "talker_decode", use_dummy_cache=True, num_layers=TALKER_NUM_LAYERS)
            self.pred_prefill_engine = self._load_engine(
                args.talker_prediction_prefill_path, self.talker_wm,
                "pred_prefill")
            self.pred_decode_engine = self._load_engine(
                args.talker_prediction_decode_path, self.talker_wm,
                "pred_decode", use_dummy_cache=True, num_layers=PREDICTOR_NUM_LAYERS)
            ...
            self.text_proj_engine = self._load_engine(
                args.text_projection_path, self.talker_wm,
                "text_projection")
            self.text_proj_static_seq = int(self.text_proj_engine.get_input_info(
                self.text_proj_engine.get_input_name(0)).shape[1])
    ...
    def _load_engine(self, path, weight_manager, tag,
                     use_dummy_cache=False, num_layers=0):
    ...
    def _bind_cache(self, prefill_engine, decode_engine):
        prefill_names = [prefill_engine.get_input_name(i)
                         for i in range(prefill_engine.get_num_inputs())]
        decode_names = [decode_engine.get_input_name(i)
    ...
    def _init_engine_meta(self):
        ...
        self.prefill_input_names = [self.prefill_engine.get_input_name(i)
                                    for i in range(self.prefill_engine.get_num_inputs())]
        self.decode_input_infos = get_input_infos(self.decode_engine)
        self.decode_input_names = [self.decode_engine.get_input_name(i)
                                   for i in range(self.decode_engine.get_num_inputs())]

        # Get prefill static seq len from first input shape
        self.prefill_seq_len = int(self.prefill_input_infos[self.prefill_input_names[0]].shape[1])
        ...
        if decode_cache_names:
            self.kv_max_length = int(self.decode_input_infos[decode_cache_names[0]].shape[2])
        else:
            self.kv_max_length = 4096

        # Visual engine shape
        vis_name = self.visual_engine.get_input_name(0)
        self.visual_shape = tuple(int(d) for d in self.visual_engine.get_input_info(vis_name).shape)

        # Audio engine
        self.audio_input_names = [self.audio_engine.get_input_name(i)
        ...
        if self.enable_audio_generation:
            ...
            self.talker_prefill_names = [self.talker_prefill_engine.get_input_name(i)
            ...
            self.talker_decode_names = [self.talker_decode_engine.get_input_name(i)
            ...
            self.pred_prefill_names = [self.pred_prefill_engine.get_input_name(i)
                                       for i in range(self.pred_prefill_engine.get_num_inputs())]
            self.pred_prefill_infos = get_input_infos(self.pred_prefill_engine)
            self.pred_decode_names = [self.pred_decode_engine.get_input_name(i)
        ...
        logger.info(f"  prefill_seq_len={self.prefill_seq_len}, kv_max={self.kv_max_length}")

    # ── Audio Encoder ─────────────────────────────────────────────────────────

    def run_audio_encoder(self, input_features: torch.Tensor,
        ...
        compact_features = input_features.permute(0, 2, 1)[
            feature_attention_mask.bool()].permute(1, 0)
        ...
        padded_feature = torch.nn.utils.rnn.pad_sequence(
            chunk_list, batch_first=True).transpose(1, 2)
        ...
        for i in range(padded_feature.shape[0]):
            ...
            self.audio_engine.set_input(self.audio_input_names[0],
                                        np.ascontiguousarray(engine_in))
            self.audio_engine.set_input(self.audio_input_names[1],
                                        np.ascontiguousarray(sc.numpy()))
    ...
    def run_visual_encoder(self, pixel_values: torch.Tensor):
        ...
        if list(pv.shape) != list(expected):
            ...
            ).to(torch.float16).reshape(expected)
        vis_name = self.visual_engine.get_input_name(0)
        ...
        for i in range(self.visual_engine.get_num_outputs()):
            ...
            outputs.append(torch.from_numpy(
                self.visual_engine.get_output(oname).numpy()).to(torch.float16))
    ...
    def run_text_prefill(self, inputs_embeds, deepstack_tensors,
                         position_ids_3d, actual_seq_len):
        ...
        for ci in range(num_chunks):
            ...
            chunk_embed = np.zeros((1, static_seq, THINKER_HIDDEN_SIZE), dtype=np.float16)
            ...
            for ds in deepstack_tensors:
                ds_pad = np.zeros((1, static_seq, THINKER_HIDDEN_SIZE), dtype=np.float16)
    ...
    def run_text_decode(self, first_token, past_seq, all_hidden,
        ...
        for step in range(self.args.max_new_tokens - 1):
            ...
            self.decode_engine.set_input(names[4], np.array([valid_length], dtype=np.int32))
            ...
            logits_np = self.decode_engine.get_output(
                self.decode_engine.get_output_name(0)).numpy()
            if self.decode_engine.get_num_outputs() > 1:
                hidden_np = self.decode_engine.get_output(
                    self.decode_engine.get_output_name(1)).numpy()
                h_t = torch.from_numpy(hidden_np).to(torch.float16)
                if h_t.ndim == 2:
                    h_t = h_t.unsqueeze(0)
    ...
    def text_proj(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return run_projection_engine(self.text_proj_engine, x, static=self.text_proj_static_seq)

    # ── Talker Prefill Context (source/role/bypass fusion) ─────────────────────

    def build_talker_prefill_context(self, input_ids, full_ids,
                                     thinker_embed, thinker_hidden, speaker_name):
        ...
        im_start_pos = torch.cat([im_start_pos, torch.tensor([full_seq.shape[0]], dtype=torch.long)])
        mm_mask = ((full_seq == AUDIO_TOKEN_ID) | (full_seq == IMAGE_TOKEN_ID)
                   | (full_seq == VIDEO_TOKEN_ID))

        # tts specials = text_projection(thinker_embed[special]) -> PROJECTED 1024.
        tts_bos_p = self.text_proj(self.text_embedding[torch.tensor([TTS_BOS_TOKEN_ID])].unsqueeze(0))
        tts_eos_p = self.text_proj(self.text_embedding[torch.tensor([TTS_EOS_TOKEN_ID])].unsqueeze(0))
        tts_pad_p = self.text_proj(self.text_embedding[torch.tensor([TTS_PAD_TOKEN_ID])].unsqueeze(0))
        ...
        for idx in range(len(im_start_pos) - 1):
            ...
            if role_token == ASSISTANT_TOKEN_ID and idx == len(im_start_pos) - 2:
                ...
                codec_embeds = self.talker_embedding[torch.tensor(codec_ids)].unsqueeze(0)

                # native assistant input_embeds = assistant_text + assistant_codec
                assistant_text = torch.cat([
                    asst_proj[:, :3, :], tts_pad_p.expand(1, 4, -1),
                    tts_bos_p, asst_proj[:, 3:4, :],
                ], dim=1)
        ...
        return (source, role_mask, bypass_embeds, bypass_mask, actual_len,
                trailing_text_hidden, tts_pad_p)

    # ── Talker + Predictor Generate ───────────────────────────────────────────

    def run_talker_generate(self, source, role_mask, bypass_embeds, bypass_mask,
                            actual_len, trailing_text_hidden, tts_pad):
        ...
        for ci in range(num_chunks):
            ...
            padded_source = np.zeros((1, talker_static, THINKER_HIDDEN_SIZE), dtype=np.float16)
            padded_role = np.zeros((1, talker_static, 1), dtype=np.float16)
            padded_bypass = np.zeros((1, talker_static, TALKER_HIDDEN_SIZE), dtype=np.float16)
            ...
            padded_bypass_mask[:, :cl, :] = bypass_mask[:, start:end, :].detach().numpy()
            past_arr = np.array([past_seq_val], dtype=np.int32)
            cl_arr = np.array([cl], dtype=np.int32)

            names = self.talker_prefill_names
            self.talker_prefill_engine.set_input(names[0], np.ascontiguousarray(padded_source))
            self.talker_prefill_engine.set_input(names[1], np.ascontiguousarray(padded_role))
            self.talker_prefill_engine.set_input(names[2], np.ascontiguousarray(padded_bypass))
            self.talker_prefill_engine.set_input(names[3], np.ascontiguousarray(padded_bypass_mask))
            ...
            talker_logits = torch.from_numpy(
                self.talker_prefill_engine.get_output(
                    self.talker_prefill_engine.get_output_name(0)).numpy())
            if self.talker_prefill_engine.get_num_outputs() > 1:
                talker_hidden = torch.from_numpy(
                    self.talker_prefill_engine.get_output(
                        self.talker_prefill_engine.get_output_name(1)).numpy())
            past_seq_val += cl

        # Sample first codec token
        first_codec = greedy_or_sample(
            talker_logits.float().numpy(),
            self.args.talker_do_sample, self.args.talker_temperature,
        ...
        for step in range(1, self.args.talker_max_new_tokens):
            ...
            primary_emb = self.talker_embedding[last_primary].reshape(1, 1, -1).to(torch.float16)
            ...
            last_res_emb = self.codec_embeddings[num_pred_heads - 1][last_res_token].reshape(1, 1, -1).to(torch.float16)
            ...
            self.talker_decode_engine.set_input(names[4], np.array([talker_past_seq], dtype=np.int32))
            ...
            d_logits = torch.from_numpy(
                self.talker_decode_engine.get_output(
                    self.talker_decode_engine.get_output_name(0)).numpy())
            d_hidden = None
            if self.talker_decode_engine.get_num_outputs() > 1:
                d_hidden = torch.from_numpy(
                    self.talker_decode_engine.get_output(
                        self.talker_decode_engine.get_output_name(1)).numpy())
    ...
    def _run_predictor_full(self, talker_hidden, primary_token_id, num_pred_heads):
        ...
        primary_embed = self.talker_embedding[primary_token_id].reshape(1, 1, -1).to(torch.float16)
        if talker_hidden is not None:
            pred_input = torch.cat([
                talker_hidden.to(torch.float16).reshape(1, 1, -1), primary_embed], dim=1)
        else:
        ...
        self.pred_prefill_engine.set_input(names[0], np.ascontiguousarray(pred_input.detach().numpy()))
        ...
        pred_logits = torch.from_numpy(
            self.pred_prefill_engine.get_output(
                self.pred_prefill_engine.get_output_name(0)).numpy())
        ...
        for g in range(1, num_pred_heads):
            codec_embed = self.codec_embeddings[g - 1][ct].reshape(1, 1, -1).to(torch.float16)
            head_mask_d = np.zeros((1, 1, num_pred_heads, 1), dtype=np.float16)
            head_mask_d[:, :, g, 0] = 1.0

            names = self.pred_decode_names
            self.pred_decode_engine.set_input(names[0], np.ascontiguousarray(codec_embed.detach().numpy()))
            self.pred_decode_engine.set_input(names[1], np.ascontiguousarray(head_mask_d))
            self.pred_decode_engine.set_input(names[2], np.array([2 + g - 1], dtype=np.int32))
            ...
            pred_logits_d = torch.from_numpy(
                self.pred_decode_engine.get_output(
                    self.pred_decode_engine.get_output_name(0)).numpy())
            pred_hidden_d = None
            if self.pred_decode_engine.get_num_outputs() > 1:
                pred_hidden_d = torch.from_numpy(
                    self.pred_decode_engine.get_output(
                        self.pred_decode_engine.get_output_name(1)).numpy())
    ...
    def synth_codes_from_text_ids(self, input_ids, inputs_embeds,
                                  talker_text_ids, hidden_states,
        ...
        """
        gen_embeds = self.text_embedding[torch.tensor(talker_text_ids)].unsqueeze(0).to(torch.float16)
        ...
        source, role_mask, bypass_embeds, bypass_mask, talker_actual_len, \
            trailing_text_hidden, tts_pad = self.build_talker_prefill_context(
        ...
        return self.run_talker_generate(
            ...
            talker_actual_len, trailing_text_hidden, tts_pad)
    ...
    def _build_rope_shim(self):
        ...
        try:
            ...
            for src in (tc, tc.get("text_config", {}), cfg):
                if isinstance(src, dict) and "position_id_per_seconds" in src:
                    pos_per_sec = int(src["position_id_per_seconds"]); break
        except Exception:
            pass
        ...
        shim.get_llm_pos_ids_for_vision = _types.MethodType(
            Qwen3OmniMoePreTrainedModelForConditionalGeneration.get_llm_pos_ids_for_vision, shim)
    ...
    def compute_position_ids(self, input_ids, attention_mask, inputs):
        ...
        if mode == "exact":
            ...
            pos, rope_deltas = Qwen3OmniMoePreTrainedModelForConditionalGeneration.get_rope_index(
                shim, input_ids, img_thw, vid_thw, attention_mask,
                use_audio_in_video=True, audio_seqlens=audio_seqlens,
        ...
        rope_deltas = (max_position_ids + 1 - attention_mask.sum(dim=-1, keepdim=True)).to(torch.long)
        return position_ids_3d[:, 0, :].to(torch.int32), rope_deltas

    # ── Main Run ──────────────────────────────────────────────────────────────

    def run(self, image=None, audio=None, prompt=None, speaker=DEFAULT_SPEAKER,
            generate_audio=None):
    ...
    def run_conversation(self, conversation, speaker=DEFAULT_SPEAKER,
                         generate_audio=None, use_audio_in_video=False):
        ...
        """
        args = self.args
        gen_audio = (self.enable_audio_generation if generate_audio is None
        ...
        deepstack_tensors = [torch.zeros(1, actual_seq_len, THINKER_HIDDEN_SIZE,
        ...
        if pv_key in inputs:
            ...
            for i, ds in enumerate(ds_list):
                dense = torch.zeros(1, actual_seq_len, THINKER_HIDDEN_SIZE, dtype=torch.float16)
        ...
        position_ids_3d, rope_deltas = self.compute_position_ids(input_ids, attention_mask, inputs)
        ...
        perf["thinker_ttft_s"] = perf["thinker_prefill_s"] + perf.get("audio_encoder_s", 0.0) + perf.get("visual_encoder_s", 0.0)
        logger.info(f"  Prefill done, first_token={first_token}, valid_length={valid_length}")
        ...
        if gen_audio:
            ...
            if clean_text != raw_text:
                ...
                clean_ids = self.tokenizer(
                    clean_text, add_special_tokens=False)["input_ids"]
            ...
            body_ids = talker_text_ids[:-len(eos_tail)] if eos_tail else talker_text_ids
            ...
            if seg:
                ...
                clean_for_seg = clean_text if clean_text else (output_text[0] if output_text else "")
                ...
                gap = torch.zeros(int(SAMPLE_RATE * args.talker_segment_gap_ms / 1000.0))
                seg_wavs = []
                total_codes = 0
                eos_one = eos_tail or [IM_END_TOKEN_ID]
                for si, seg in enumerate(segments):
                    seg_ids = self.tokenizer(seg, add_special_tokens=False)["input_ids"] + eos_one
                    seg_codes = self.synth_codes_from_text_ids(
                        input_ids, inputs_embeds, seg_ids, hidden_states, speaker=speaker)
                    total_codes += int(seg_codes.shape[2])
                    seg_wav = self.run_code2wav(seg_codes).float().cpu().flatten()
                    seg_wavs.append(seg_wav)
                    if si < len(segments) - 1:
                        seg_wavs.append(gap)
                    logger.info(f"  segment {si+1}/{len(segments)}: "
                ...
                codes = self.synth_codes_from_text_ids(
                    input_ids, inputs_embeds, body_ids + (eos_tail or [IM_END_TOKEN_ID]),
                    hidden_states, speaker=speaker)
                perf["talker_s"] = time.perf_counter() - _t
...
def main():
    ...
    if args.run_device_num < 2 and args.enable_audio_generation:
        logger.warning("Running Omni on a single device requires 48GB of memory; otherwise, multiple devices are needed.")
    ...
    if example_mode == "omni":
        ...
        result = pipeline.run(image=image, audio=audio, prompt=prompt,
                              speaker=DEFAULT_SPEAKER)
        ...
        conversation.append({"role": "assistant",
                             "content": [{"type": "text", "text": answer0}]})
