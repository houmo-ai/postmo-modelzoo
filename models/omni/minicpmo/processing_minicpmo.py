...
def validate_audio_parts(audios_list: List[List[np.ndarray]], audio_parts: Optional[List[List[int]]]):
    ...
    if len(audio_parts) != len(audios_list):
        raise ValueError("audio_parts length mismatch")
...
def merge_audio_parts(audios: List[np.ndarray], parts: Optional[List[int]]) -> List[np.ndarray]:
    ...
    for i, audio in enumerate(audios):
        same_part = i == 0 or parts[i] == parts[i-1]
        if same_part:
            current.append(audio)
        else:
            merged.append(np.hstack(current))
            current = [audio]
    if current:
        merged.append(np.hstack(current))
    return merged
...
def chunk_audio(audio: np.ndarray, max_len: int) -> List[np.ndarray]:
    ...
    if len(audio) <= max_len:
        return [audio]
    chunks = []
    n_chunks = math.ceil(len(audio) / max_len)
    for i in range(n_chunks):
        chunks.append(audio[i*max_len:(i+1)*max_len])
    return chunks
...
def pad_inputs(items, dim, padding_side, tensor: torch.Tensor):
    padding_length = []
...
class MiniCPMOProcessor(ProcessorMixin):
    ...
    def audio_feature_extract(
        ...
        **kwargs,
        ...
        audios_list = normalize_audios(audios)
        validate_audio_parts(audios_list, audio_parts)
        ...
        for idx, audios_ in enumerate(audios_list):
            ...
            for a in merged:
                if max_audio_len:
                    final_audios.extend(chunk_audio(a, max_audio_len))
                else:
        ...
        if audio_features_all:
            audio_features_all = [f.permute(1, 0) for f in audio_features_all]
            audio_features = torch.nn.utils.rnn.pad_sequence(audio_features_all, batch_first=True, padding_value=0.0).permute(0, 2, 1)
        else:
            audio_features = []
    ...
    def convert_text_to_list(self, 
        ...
        return input_ids_list, image_bounds_list, audio_bounds_list, spk_bounds_list
    ...
    def _convert_omni_to_inputs(
        ...
        **kwargs,
        ...
        input_ids_list, image_bounds_list, audio_bounds_list, spk_bounds_list = self.convert_text_to_list(
            texts,
            split_pattern,
            image_pattern,
            audio_pattern,
