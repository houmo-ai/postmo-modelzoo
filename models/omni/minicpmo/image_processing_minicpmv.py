...
class MiniCPMVImageProcessor(BaseImageProcessor):
    ...
    def __init__(self, max_slice_nums=1, scale_resolution=448, patch_size=14, **kwargs):
        ...
        self.im_start_token = "<image>"
        self.im_end_token = "</image>"
    ...
    def get_sliced_images(self, image, max_slice_nums=None):
        ...
        source_image, patches, _ = self.slice_image(
    ...
    def preprocess(
        ...
        images_list = self._normalize_input_list(images)

        new_images_list, image_sizes_list, tgt_sizes_list = [], [], []

        for _images in images_list:
            new_images, image_sizes, tgt_sizes = self._process_image_group(_images, max_slice_nums)
    ...
    def _process_image_group(self, images, max_slice_nums):
        ...
        images = [self.to_pil_image(img).convert("RGB") for img in images]
        input_data_format = infer_channel_dimension_format(np.array(images[0]))

        new_images, tgt_sizes = [], []
        image_sizes = [img.size for img in images]

        for image in images:
            image_patches = self.get_sliced_images(image, max_slice_nums)
            image_patches = [to_numpy_array(p).astype(np.float32) / 255 for p in image_patches]
            image_patches = [
                self.normalize(img, mean=self.mean, std=self.std, input_data_format=input_data_format)
