...
class HmQwen:
    ...
    def get_model_input_shape(
        self, runtime_model, input_name: str
    ) -> tuple[int, ...]:
    ...
    def chat(self, question, image_path=None):
        ...
        for round_idx in range(prefill_loop_round):
            ...
            self.set_model_input(
                self.prefill, time_position_ids_name, chunk_pos[0:1]
            )
            self.set_model_input(
                self.prefill, hight_position_ids_name, chunk_pos[1:2]
            )
            self.set_model_input(
                self.prefill, width_position_ids_name, chunk_pos[2:3]
            )
            self.set_model_input(
