...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="/data02/datasets/Qwen3.5-9B", help="source HF model directory")
    parser.add_argument(
        "--out",
        type=str,
        default="/data02/datasets/Qwen3.5-9B-rotated-fp",
...
def main():
    ...
    if effective_vision_rotation == "full" and should_rotate_llm:
        ...
        print("[3/5] Fusing vision norms and rotating full vision tower only (LLM rotation skipped) ...")
        model = fuse_vision_layer_norms_qwen3_5_vl(
            model=model,
            pre_lm_head_norm_module_name=pre_lm_head_norm_module_name,
            layers_node=layers_node,
    ...
    if effective_vision_rotation == "full":
        model, _ = rotate_vision_model_qwen3_5_vl(
            model=model,
            rotate_mode="hadamard" if args.llm_rotation == "none" else args.llm_rotation,
