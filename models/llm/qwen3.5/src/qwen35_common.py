...
def build_calibration_dataset(args):
    if getattr(args, "calibration_jsonl", None):
        ds = get_jsonl_texts(
            args.calibration_jsonl, args.nsamples, args.calibration_text_key
        )
...
def model_type_of(model_path: str, trust_remote_code: bool) -> str:
    return str(
        getattr(load_config(model_path, trust_remote_code), "model_type", "")
    ).lower()
...
def text_num_hidden_layers(model_path: str, trust_remote_code: bool) -> int:
    ...
    if (
        ...
        and getattr(text_cfg, "num_hidden_layers", None) is not None
    ):
...
def layer_limit_dynamic_skips(
    model_path: str, trust_remote_code: bool, max_quant_layers: Optional[int]
):
    ...
    if max_quant_layers < 0 or max_quant_layers > num_layers:
        raise ValueError(
            f"--max-quant-layers must be between 0 and {num_layers}, got {max_quant_layers}"
        )
    return {
        rf"-:.*\.layers\.{idx}\..*": {} for idx in range(max_quant_layers, num_layers)
    }
...
def has_vision_model(model_path: str, trust_remote_code: bool) -> bool:
    cfg = load_config(model_path, trust_remote_code)
    return model_type_of(
        model_path, trust_remote_code
    ) in FULL_VISION_MODEL_TYPES and hasattr(cfg, "vision_config")


def resolve_rotation_vision(
    model_path: str, rotation: Optional[str], trust_remote_code: bool
) -> Optional[str]:
    return (
        rotation
        if rotation and has_vision_model(model_path, trust_remote_code)
        else None
    )


@torch.inference_mode()
def calculate_avg_ppl(
    model, tokenizer, device=None, nsamples: int = 8, seqlen: int = 512
):
...
def temporary_ppl_device(model, target_device: Optional[str]):
    ...
    if target_device == "auto":
        target_device = "cuda" if torch.cuda.is_available() else str(original_device)
...
def ensure_processor_saved(
    src_model_dir: str, dst_model_dir: str, trust_remote_code: bool
) -> None:
    if not has_vision_model(src_model_dir, trust_remote_code):
        return
    processor = AutoProcessor.from_pretrained(
        src_model_dir, trust_remote_code=trust_remote_code
    )
    processor.save_pretrained(dst_model_dir)


def rotate_mtp_after_save(
    src_model_dir: str,
    dst_model_dir: str,
    gptq_model,
    is_moe: bool,
    ...
    if q_block is None:
        raise RuntimeError(
            "LLM rotation was requested but no Q_block was exposed for MTP rotation."
        )
...
def ensure_visual_tensors_present(src_model_dir: str, dst_model_dir: str) -> None:
    ...
    visual_keys = sorted(
        k for k in src_wm if k.startswith("visual.") or k.startswith("model.visual.")
    )
    if not visual_keys:
        return
    ...
    if not missing:
        print(
            f"[visual] all {len(visual_keys)} visual tensors already present in {dst_model_dir}"
        )
        return

    print(
        f"[visual] {len(missing)}/{len(visual_keys)} visual tensors missing — copying passthrough sidecar"
    )
    ...
    save_file(
        state,
        sidecar_path,
        metadata={"format": "pt", "produced_by": "qwen35_common.visual_passthrough"},
    )
    for key in missing:
        dst_wm[key] = sidecar_name
    meta = dst_idx.setdefault("metadata", {})
    try:
        meta["total_size"] = int(meta.get("total_size", 0)) + os.path.getsize(
            sidecar_path
        )
    except OSError:
        pass
    with open(dst_idx_path, "w", encoding="utf-8") as f:
        ...
        f.write("\n")
    print(
        f"[visual] wrote {len(missing)} tensors to {sidecar_name}; index.json patched"
    )
...
def is_gptq_quantized_dir(model_dir: str) -> bool:
    ...
    return (
        ...
        and str(quant_config.get("quant_method", "")).lower() == "gptq"
    )
...
def vision_demo_outputs(
    ...
    if is_gptq_quantized_dir(model_dir):
        ...
        wrapper = GPTQModel.load(
            model_dir, trust_remote_code=trust_remote_code, **load_kwargs
        )
        model = wrapper.model
    else:
        load_kwargs = (
            {"device_map": "auto"} if device == "auto" else {"device_map": "cpu"}
        )
    ...
    batch = move_batch_to_device(
        batch, device=runtime_device, float_dtype=forward_dtype
    )
...
def _decode_generated_suffix(
    processor, generated: torch.Tensor, prompt_len: int
) -> str:
...
def _format_first_token_diff(
    processor,
    ref_generated: torch.Tensor,
    test_generated: torch.Tensor,
    prompt_len: int,
) -> str:
    tokenizer = _processor_tokenizer(processor)
    ...
    for idx, (ref_id, test_id) in enumerate(zip(ref_suffix, test_suffix)):
        if ref_id != test_id:
            ref_piece = tokenizer.decode(
                [ref_id], skip_special_tokens=False, clean_up_tokenization_spaces=False
            )
            test_piece = tokenizer.decode(
                [test_id], skip_special_tokens=False, clean_up_tokenization_spaces=False
            )
            return (
...
def validate_vision_demo_pair(
    ...
    *,
    ...
    processor = AutoProcessor.from_pretrained(
        ref_model_dir, trust_remote_code=trust_remote_code
    )
    batch = build_vision_demo_inputs(processor, prompt, image_size, seed)
    ref_logits, ref_generated = vision_demo_outputs(
        ref_model_dir, batch, device, torch_dtype, trust_remote_code, generation_tokens
    )
    test_logits, test_generated = vision_demo_outputs(
        test_model_dir, batch, device, torch_dtype, trust_remote_code, generation_tokens
    )
    max_abs = (test_logits - ref_logits).abs().max().item()
    mean_abs = (test_logits - ref_logits).abs().mean().item()
    last_token_equal = bool(
        torch.equal(test_logits[:, -1].argmax(dim=-1), ref_logits[:, -1].argmax(dim=-1))
    )
    prompt_len = batch["input_ids"].shape[1]
    generation_equal = bool(
        torch.equal(test_generated[:, prompt_len:], ref_generated[:, prompt_len:])
    )
    print(
    ...
    token_diff = _format_first_token_diff(
        processor, ref_generated, test_generated, prompt_len
    )
    ...
    if (
        require_equal
        and not torch.allclose(test_logits, ref_logits, atol=atol, rtol=rtol)
        and not generation_equal
    ):
