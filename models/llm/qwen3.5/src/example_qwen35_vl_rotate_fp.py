...
#
# File: example_qwen35_vl_rotate_fp.py
# Description:
#   Example script to rotate Qwen3.5-VL fp weights and validate functional equivalence.
#
...
def parse_args():
    parser = argparse.ArgumentParser(description="Rotate Qwen3.5 VL LLM weights and align vision output projection")
    parser.add_argument("--model", type=str, required=True, help="source HF model directory")
    parser.add_argument("--out", type=str, required=True, help="output HF model directory")
    parser.add_argument(
        "--llm-rotation",
        type=str,
        default="hadamard",
        choices=["hadamard", "block_hadamard", "random"],
        help="rotation mode for LLM weights",
    )
    parser.add_argument(
        "--vision-rotation",
        type=str,
    ...
    parser.add_argument("--device", type=str, default="cuda:0", help="device used to materialize rotation matrices")
    parser.add_argument(
        "--torch-dtype",
        type=str,
        default="auto",
    ...
    parser.add_argument("--validate-image-size", type=int, default=224, help="synthetic validation image size")
    parser.add_argument("--validate-prompt", type=str, default=DEFAULT_PROMPT, help="validation prompt")
    parser.add_argument(
    ...
    parser.add_argument("--atol", type=float, default=5e-2, help="absolute tolerance for validation")
    parser.add_argument("--rtol", type=float, default=5e-2, help="relative tolerance for validation")
    parser.add_argument(
        "--max-shard-size",
        type=str,
        default="5GB",
...
def build_validation_inputs(processor, prompt: str, image_size: int, seed: int):
    ...
    image = Image.fromarray(generator.integers(0, 256, size=(image_size, image_size, 3), dtype=np.uint8))
    messages = [
        {
            "role": "user",
            "content": [
...
def generate_tokens(model, batch, device: str, max_new_tokens: int, forward_dtype: torch.dtype | None = None):
    original_dtype = next(model.parameters()).dtype
    effective_dtype = forward_dtype or original_dtype
    model = model.to(device=device, dtype=effective_dtype)
    batch = move_batch_to_device(batch, device=device, float_dtype=effective_dtype)
...
def validate_equivalence(model, processor, args):
    ...
    ref_logits = compute_logits(model, batch, validate_device, forward_dtype=validate_dtype)
...
def compare_logits(model, batch, ref_logits, ref_generated, args):
    ...
    rotated_logits = compute_logits(model, batch, validate_device, forward_dtype=validate_dtype)
    max_abs_diff = (rotated_logits - ref_logits).abs().max().item()
    mean_abs_diff = (rotated_logits - ref_logits).abs().mean().item()
    last_token_equal = bool(torch.equal(rotated_logits[:, -1].argmax(dim=-1), ref_logits[:, -1].argmax(dim=-1)))
    ...
    print(f"Validation full_logits max_abs_diff={max_abs_diff:.6e}, mean_abs_diff={mean_abs_diff:.6e}")
    print(f"Validation last_token_equal={last_token_equal}")
    print(f"Validation generation_equal={generation_equal}, generated_tokens={args.validate_generation_tokens}")
...
def main():
    ...
    ref_generated = None
    if args.validate:
        validation_batch, ref_logits, ref_generated = validate_equivalence(model, processor, args)

    if args.vision_rotation == "full":
        print("[3/5] Fusing vision/text norms, rotating full vision tower, and rotating LLM ...")
        ...
        print("[3/5] Fusing text norms, rotating LLM, and aligning vision output projection ...")
