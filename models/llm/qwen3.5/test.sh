...
if should_run_step "demo"; then
    ...
    if command -v llm_perf &>/dev/null; then
        ...
        llm_perf --model_name "${MODEL_NAME}-${MODEL_SIZE}" --devices "${devices_param}" \
            --input 256 --output 64 --loop 1 --batch 1 ${LOAD_MODE} \
            --prefill "output/${HOUMO_TARGET}/${MODEL_NAME}_prefill.hmm" \
            --decode "output/${HOUMO_TARGET}/${MODEL_NAME}_decode.hmm" \
            --visual "output/${HOUMO_TARGET}/${MODEL_NAME}_visual_896x896x2.hmm" \
