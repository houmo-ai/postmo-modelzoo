...
if [ "$STEP" = "all" ] || [ "$STEP" = "build" ]; then
    if [[ "$MODEL_TYPE" == "precompiled" ]]; then
        ...
        PACKAGE_PATTERN_Q=hmquant
        FOUND_PACKAGE_Q=0
        echo "================================"
        echo "Checking python3 package: $PACKAGE_PATTERN_Q"
        if command -v python3 &>/dev/null && command -v pip3 &>/dev/null; then
            if pip3 list --format=columns 2>/dev/null | grep -E "^$PACKAGE_PATTERN_Q" >/dev/null 2>&1; then
                echo "✓ Found python3 package: $PACKAGE_PATTERN_Q"
                pip3 list --format=columns 2>/dev/null | grep -E "^$PACKAGE_PATTERN_Q" | while read -r line; do
                    echo "  - $line"
                done
                FOUND_PACKAGE_Q=1
            else
                echo "✗ Not found package: $PACKAGE_PATTERN_Q"
            fi
        else
        ...
        if [[ "$FOUND_PACKAGE" -eq 1 && "$FOUND_PACKAGE_Q" -eq 1 && "$FOUND_GPU" -eq 1 ]]; then
