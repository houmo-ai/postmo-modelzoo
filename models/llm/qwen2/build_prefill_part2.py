...
def build(args=None):
    ...
    part_name = f"{model_name}_prefill_part2"
    quant_name = f"hmquant_{model_name}_part2_with_act"
    ...
    if stage == 'build' or stage == 'all':
        ...
        tcim.build.build_from_hmonnx(onnx_model, weights=data_dict, model_name=part_name, compiler_cfg=compile_config,
                                     inputs=input_cfg, hdplcc_options=["-O2"], const_weight_prefix=f"{model_name}_part2_", lite=True)
