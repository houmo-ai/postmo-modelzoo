...
# File: build.py
# Description:
#   Qwen-Agentworld Model Build and Test Tool - Python script for building and testing
...
def sanitize_name(name: str) -> str:
    """
    Sanitizeizes a model name by replacing colons and slashes with underscores.
    This is necessary to make the name compatible with HOUMO model naming conventions.
    """
...
def _validate_adjust_flash_attention(flash_val: int, context_length: int) -> int:
    """Validates and adjusts FlashAttention parameter values.

    qwen-agentworld is a text-only LLM (language_model_only), so there is no
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=2,
        choices=[0, 1, 2],
        help="FlashAttention optimization for prefill/decode: 0=off, 1=graph level, 2=operator level",
    )
    parser.add_argument(
...
def _load_golden_output(model_dir, output_name, batch) -> tuple[np.ndarray, str]:
    ...
    return golden_output, output_data_path


def _compare_output(output_name, output_data, golden_output) -> bool:
...
def test(model_name, model_dir, output_dir, profile, batch=1):
    ...
    for id in range(output_num):
        ...
        print(
            f"output_info[{output_name}] shape = {output_info.shape}, "
            f"dtype = {output_info.dtype}, format = {output_info.format.name}"
        )
        start = time.time()
