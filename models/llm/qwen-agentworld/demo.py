...
# File: demo.py
# Description:
#   Qwen-Agentworld Inference Demo - Python script for running Qwen-Agentworld
#   Inference on HOUMO AI device (text only, no vision).
#
...
class HmQwen:
    ...
    def __init__(
        ...
        prefill_output_names = {
            self.prefill.get_output_name(j)
            for j in range(self.prefill.get_num_outputs())
        }
        decode_output_names = {
            self.decode.get_output_name(j)
            for j in range(self.decode.get_num_outputs())
        }
        ...
        for i in range(self.prefill.get_num_inputs()):
            ...
            if "conv_cache" in input_name:
                ...
                if output_name in prefill_output_names:
                    ...
                    self.decode.set_dev_input(input_name, cache)
                    if output_name in decode_output_names:
        ...
        self.tokenizer = build_tokenizer(tokenizer_dir=tokenizer_dir)
