...
def get_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--stage", dest="stage", type=str, default="s2t", choices=("s2t", "s2s", "e2e"), help="inference pipeline"
    )
    ...
    parser.add_argument(
        ...
        dest="tokenizer_dir",
        type=Path,
        default=ROOT / "Fun-Audio-Chat-8B",
        help="processor and tokenizer directory",
    )
    parser.add_argument(
        "--embedding_path",
        dest="embedding_path",
        type=Path,
        default=HMQUANT_DIR / "quant_embedding.pt",
        help="text embedding weights",
    )
    parser.add_argument(
        "--audio_embedding_path",
        dest="audio_embedding_path",
        type=Path,
        default=HMQUANT_DIR / "quant_audio_embedding.pt",
        help="audio token embedding weights",
    )
    parser.add_argument(
        "--pre_matching_path",
        dest="pre_matching_path",
        type=Path,
        default=HMQUANT_DIR / "audio_decoder_pre_matching.pt",
        help="audio decoder pre-matching weights",
    )
    parser.add_argument(
        "--flow_input_embedding_path",
        dest="flow_input_embedding_path",
        type=Path,
        default=HMQUANT_DIR / "flow_input_embedding.pt",
        help="Flow input embedding weights",
    )
    parser.add_argument(
        "--speaker_info_path",
        dest="speaker_info_path",
        type=Path,
        default=HMQUANT_DIR / "new_spk2info.pt",
        help="default speaker profile",
    )
    parser.add_argument(
        "--audio_encoder_path",
        dest="audio_encoder_path",
        type=Path,
        default=MODEL_DIR / "funaudiochat-8b_audio_encoder.hmm",
        help="audio encoder HMM",
    )
    parser.add_argument(
        "--prefill_path",
        dest="prefill_path",
        type=Path,
        default=MODEL_DIR / "funaudiochat-8b_prefill.hmm",
        help="language prefill HMM",
    )
    parser.add_argument(
        "--decode_path",
        dest="decode_path",
        type=Path,
        default=MODEL_DIR / "funaudiochat-8b_decode.hmm",
        help="language decode HMM",
    )
    parser.add_argument(
        "--audio_tower_path",
        dest="audio_tower_path",
        type=Path,
        default=MODEL_DIR / "funaudiochat-8b_audio_tower.hmm",
        help="audio tower HMM",
    )
    parser.add_argument(
        "--audio_decoder_prefill_path",
        dest="audio_decoder_prefill_path",
        type=Path,
        default=MODEL_DIR / "funaudiochat-8b_audio_decoder_prefill.hmm",
        help="CRQ prefill HMM",
    )
    parser.add_argument(
        "--audio_decoder_decode_path",
        dest="audio_decoder_decode_path",
        type=Path,
        default=MODEL_DIR / "funaudiochat-8b_audio_decoder_decode.hmm",
        help="CRQ decode HMM",
    )
    parser.add_argument(
        "--flow_encoder_path",
        dest="flow_encoder_path",
        type=Path,
        default=MODEL_DIR / "cosyvoice3-0.5b-2512_flow_encoder.hmm",
        help="Flow encoder HMM",
    )
    parser.add_argument(
        "--flow_spk_path",
        dest="flow_spk_path",
        type=Path,
        default=MODEL_DIR / "cosyvoice3-0.5b-2512_flow_spk.hmm",
        help="Flow speaker HMM",
    )
    parser.add_argument(
        "--flow_decoder_path",
        dest="flow_decoder_path",
        type=Path,
        default=MODEL_DIR / "cosyvoice3-0.5b-2512_flow_decoder.hmm",
        help="Flow decoder HMM",
    )
    parser.add_argument(
        "--hift_part1_path",
        dest="hift_part1_path",
        type=Path,
        default=MODEL_DIR / "cosyvoice3-0.5b-2512_hift_part1.hmm",
        help="HiFT part1 HMM",
    )
    parser.add_argument(
        "--hift_part2_path",
        dest="hift_part2_path",
        type=Path,
        default=MODEL_DIR / "cosyvoice3-0.5b-2512_hift_part2.hmm",
        help="HiFT part2 HMM",
    )
    parser.add_argument(
        "--vad_path", dest="vad_path", type=Path, default=MODEL_DIR / "fsmd_vad.hmm", help="FSMN VAD HMM"
    )
    parser.add_argument(
        "--config_path", dest="config_path", type=Path, default=HMQUANT_DIR / "config.yaml", help="VAD config"
    )
    parser.add_argument("--cmvn_path", dest="cmvn_path", type=Path, default=HMQUANT_DIR / "am.mvn", help="VAD CMVN")
    parser.add_argument(
        "--system_prompt", dest="system_prompt", type=str, default=None, help="system prompt; default depends on stage"
    )
    parser.add_argument(
        "--max_new_tokens", dest="max_new_tokens", type=int, default=2048, help="maximum generated text tokens"
    )
    parser.add_argument(
        "--temperature", dest="temperature", type=float, default=0.6, help="speech sampling temperature"
    )
    ...
    parser.add_argument(
        "--repetition_penalty", dest="repetition_penalty", type=float, default=1.2, help="speech repetition penalty"
    )
    ...
    parser.add_argument(
        "--no_force_audio_bos",
        dest="no_force_audio_bos",
        type=_parse_bool,
        default=False,
        nargs="?",
        const=True,
        help="disable forced text audio BOS",
    )
    parser.add_argument(
        "--token_hop", dest="token_hop", type=int, default=125, help="new codec tokens per token2wav chunk"
    )
    parser.add_argument(
        "--token_overlap", dest="token_overlap", type=int, default=3, help="history codec tokens per non-first chunk"
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        const=True,
        help="enable performance reporting; pass false to disable",
    )
    parser.add_argument(
        "--output_wav",
        dest="output_wav",
        type=Path,
        default=ROOT / "result" / "turn_000_response.wav",
        help="S2S response WAV",
    )
    parser.add_argument(
        "--e2e_output_dir", dest="e2e_output_dir", type=Path, default=ROOT / "result", help="E2E output directory"
    )
    return parser
...
class HmFunAudioChat:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        paths = FunAudioChatPaths(
            **{name: getattr(args, name) for name in path_names if name not in ("audio_path", "output_wav")}
        )
            ...
            turns.append(
                {
                    ...
                    "response_wav": str(response_path),
                    "speech_token_count": len(event.speech_ids),
                }
            )
