...
public:
     ...
     * @param use_v1 Whether to use v1 mode (aspect ratio preservation) or v2 (direct resize)
     */
    HmImageProcessor(int target_width = 448, int target_height = 448, bool use_v1 = true);
     ...
     */
    std::vector<ProcessedImage> LoadAndProcessBatch(const std::vector<std::string> &image_paths);

    /**
     * @brief Convert processed image to half precision tensor (NCHW format, YUV color space)
     * @param image Processed image
     * @return Vector of half precision values in NCHW format (YUV444)
     */
    std::vector<half_float::half> ToHalfTensor(const ProcessedImage &image);
