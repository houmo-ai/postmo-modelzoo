...
public:
    ...
    float elapsed_ms() const {
        return std::chrono::duration<float, std::milli>(end_time_ - start_time_).count();
    }
private:
...
public:
     ...
     */
    std::string Chat(const std::vector<std::string> &image_paths, const std::string &prompt);
...
private:
    ...
    void VisionInfer();
    std::tuple<std::vector<half_float::half>,
    ...
    void PrefillSetInputDatas(
        ...
        const std::vector<int32_t> &width_position_ids,
