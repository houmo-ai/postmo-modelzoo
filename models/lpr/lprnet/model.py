...
class LPRNet(BaseModel):
    ...
    def demo(self, filepaths: list):
        ...
        for idx, filepath in enumerate(filepaths):
            ...
            logger.info(f"image => {file_name} have {len(plate_strs_list)} license plates, numbers: {dump_str}")
    ...
    def evaluate(self, dataset, num=0):
        ...
        t2=time.time()
        logger.info(f"Test Speed: {(t2 - t1) / len(img_path_list)}s 1/{len(img_path_list)}")