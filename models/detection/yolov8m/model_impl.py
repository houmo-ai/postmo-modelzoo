...
from hmtool.utils.postprocess import non_max_suppression2, scale_coords
...
class YoloV8(BaseModel):
    def __init__(self, **kwargs):
        ...
        self.anchor_points, self.stride_tensor = \
            self.make_anchors(self.feats, self.strides)
        self.to_coco91 = True
    ...
    def postprocess(self, outs: Dict[str, np.ndarray], in_datas: Dict[str, np.ndarray]) -> Any:
        ...
        outputs = non_max_suppression2(pred, self.conf_threshold, self.iou_threshold)
        output = outputs[0]
