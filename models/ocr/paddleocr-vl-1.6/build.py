...
def test(model_name, model_dir, output_dir, profile, batch=1, prefix=None):
    ...
    for id in range(output_num):
        ...
        if os.path.exists(output_data_path):
            ...
            continue
        if not _compare_output(output_name, golden_output, output_data):
            result_check = False
...
def _compare_output(output_name, golden_output, output_data):
    ...
    return is_match or cosine_dist >= GOLDEN_THRESH
...
if __name__ == "__main__":
    ...
    visual_model_name = f"{model_name}-{model_size}_visual_" f"{args.max_size_w}x{args.max_size_h}"
