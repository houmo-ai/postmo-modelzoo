...
if HOUMO_TARGET == "xh1":
    ...
    def parse_args():
        ...
        parser.add_argument(
            "--model_path", type=str, default="./paddleocr_rec-sim.onnx"
        )
        parser.add_argument("--output_path", default="./output/", type=str)
        parser.add_argument(
            "--calibset_path",
            type=str,
            default="CCPD2020/quant_data/rec/",
            help="quant calib data path",
        )
        parser.add_argument(
            "--precision",
            type=str,
            default="int16",
            help="quant precision, xh1 support int16, auto or int8",
        )
        parser.add_argument(
            "--calib_num", type=int, default=200, help="calibset use number"
        )
        parser.add_argument(
            "--compile", action="store_true", help="compile quanted model or no"
        )
        parser.add_argument(
            "--enable_upload",
            action="store_true",
            help="Compress hmm model and upload or no",
        )
        parser.add_argument(
            "--check_golden",
            action="store_true",
            help="check golden after compile or not",
        )
    ...
    def quantize(args):
        ...
        calibset_path = (
            os.path.join(HOUMO_DATASETS_PATH, args.calibset_path)
            if not os.path.isabs(args.calibset_path) and "./" != args.calibset_path[:2]
            else args.calibset_path
        )
        ...
        for idx, file_dir in enumerate(calibdata_list):
            ...
            sequence_data, crop_info = xh_preprocess(
                image, input_infos_list[0]["input_shape"][2:]
            )
            input_data.append([sequence_data, crop_info])

        onnx_data = onnx_preprocess(
            cv2.imread(calibdata_list[0]), input_infos_list[0]["input_shape"][2:]
        )

        qcfg = get_xh1_quant_cfg(
            args.precision,
            input_infos_list[0]["name"],
            input_infos_list[0]["input_shape"][2:],
        )

        single_batch_sequence_input = {
            input_infos_list[0]["name"]: input_data[0][0],
        ...
        save_path = os.path.join(
            args.output_path, f"{HOUMO_TARGET}/{SUB_QUANT_PATH[HOUMO_TARGET]}"
        )
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        generate_golden(
            sequencer, single_batch_sequence_input, save_path, args.model_name
        )
        sequencer.save_pkl(save_path, args.model_name)

        msg_mean = qcfg["inputs_cfg"][input_infos_list[0]["name"]][
            "first_layer_weight_denorm_mean"
        ]
        msg_mean = [msg_mean[idx] * 255 for idx in range(len(msg_mean))]
        msg_std = qcfg["inputs_cfg"][input_infos_list[0]["name"]][
            "first_layer_weight_denorm_std"
        ]
        ...
        CUSTOM_MSG[input_infos_list[0]["name"]] = {
            ...
            "input_cfg": {
                "shape": input_infos_list[0]["input_shape"],
                "data_format": qcfg["inputs_cfg"][input_infos_list[0]["name"]][
                    "data_format"
                ],
                "mean": msg_mean,
                "std": msg_std,
    ...
    def calculate_golden_infer_output(
        infer_output: Tensor, golden_path: str, model_name: str, output_name: str
    ):
        peat_name = output_name.replace("/", "_")
        golden_fix_output_dir = os.path.join(
            golden_path, f"hmquant_{model_name}_{peat_name}_output.npy"
        )
        ...
        if re_flag:
            ...
            golden_fp32_output_dir = os.path.join(
                golden_path, f"hmquant_{model_name}_{output_name}_dequant_output.npy"
            )
    ...
    def compile_model(args):
        ...
        quanted_model_dir = os.path.join(
            base_path,
            f"{SUB_QUANT_PATH[HOUMO_TARGET]}/hmquant_{args.model_name}_with_act.onnx",
        )
        ...
        hmm_path = os.path.join(
            args.output_path, f"{HOUMO_TARGET}/{args.model_name}_xh1_b1_1core_O2.hmm"
        )
    ...
    def parse_args():
        ...
        parser.add_argument(
            "--calib_num", type=int, default=1, help="calibset use number"
        )
        parser.add_argument(
            "--mix_search_cfg",
            type=str,
            default="./xh2_mix_search_cfg.yaml",
            help="quant mix search config path",
        )
        parser.add_argument(
            "--compile", action="store_true", help="compile quanted model or no"
        )
    ...
    def quantize(args):
        ...
        quant_scheme = QuantScheme(
            target_device=DeviceType.XH2a, quant_type=args.precision
        )
        quant_config = create_quant_config(quant_scheme)

        input_infos_list, output_infos_list = get_net_input_output_infos(
            args.model_path
        )
        ...
        try:
            calibset_path = (
                os.path.join(HOUMO_DATASETS_PATH, args.calibset_path)
                if not os.path.isabs(args.calibset_path)
                and "./" != args.calibset_path[:2]
                else args.calibset_path
            )
            if not os.path.exists(calibset_path):
                logger.error(f"{calibset_path} is not exist!")
    ...
    def calculate_golden_infer_output(
        ...
        peat_name = output_name.replace("/", "_")
        golden_output_dir = os.path.join(
            golden_path, f"hmquant_{model_name}_{peat_name}_output.npy"
        )
    ...
    def compile_model(args):
        ...
        hmm_path = os.path.join(
            args.output_path,
            f"{HOUMO_TARGET}/{args.model_name}_{HOUMO_TARGET}_b1_1core_O2.hmm",
        )
        if args.enable_upload:
            logger.info("Compressing hmmodel...")
...
def compare_hmm_golden(args):
    ...
    for idx, in_name in enumerate(input_names):
        golden_input_path = os.path.join(
            quanted_path, f"hmquant_{args.model_name}_{in_name}_input.npy"
        )
    ...
    for i in range(1):
        ...
        logger.info(
            "current run iter cost: %.4f, global_avg cost: %.4f."
            % (cost_time, total_time / (i + 1))
        )

    for idx, out_name in enumerate(out_names):
        infer_output = module.get_output(out_name)
        normal, cos_dist = calculate_golden_infer_output(
            infer_output, quanted_path, args.model_name, out_name
        )
        if not normal and cos_dist < NORAM_DIST[HOUMO_TARGET]:
            logger.warning(
                f"Output '{out_name}' golden compare is failed! cosine_similarity = %.6f"
                % cos_dist
            )
        else:
            logger.info(
                f"Output '{out_name}' golden compare is successful! cosine_similarity = %.6f"
                % cos_dist
            )
