...
# File: model.py
# Description:
#   ppocrv3 detection model implementation on HOUMO AI device.
#
...
class OCRDet(BaseModel):
    ...
    def demo(self, dataloader):
        ...
        for idx in range(len(dataloader)):
            ...
            if cv_image is None:
                logger.fatal("OCRDet demo requires sample['meta']['image']")
            ...
            for box_idx, box in enumerate(boxes):
                ...
                logger.info(
                    f"License plate location {box_idx}: [({box[0][0]}), ({box[1][0]}), ({box[2][0]}), ({box[3][0]})]"
                )
    ...
    def evaluate(self, dataloader, num=0):
        ...
        for idx in range(total):
            ...
            if cv_image is None or label is None:
                logger.fatal("OCRDet eval requires sample['meta']['image'] and sample['meta']['label']")
            ...
            data_list = [
                np.stack((dst_data[key_str],), axis=0)
                for key_str in ["shape", "polys", "ignore_tags"]
            ]
            post_result = self.run(sample)
