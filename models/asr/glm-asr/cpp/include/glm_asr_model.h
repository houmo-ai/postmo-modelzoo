...
#ifndef HOUMO_GLM_ASR_MODEL_H
#define HOUMO_GLM_ASR_MODEL_H
...
class GlmAsrContext : public ASRContext {
 public:
  explicit GlmAsrContext(ASRModel* model, int n_ctx);
  ~GlmAsrContext() override = default;

  GlmAsrContext(const GlmAsrContext&) = delete;
  GlmAsrContext& operator=(const GlmAsrContext&) = delete;
  ...
  std::vector<float16> Encode(const std::vector<float>& mel_features, int n_mels,
...
class GlmAsrModel : public ASRModel {
 public:
  explicit GlmAsrModel(const ModelConfig& config);
  ~GlmAsrModel();

  GlmAsrModel(const GlmAsrModel&) = delete;
  GlmAsrModel& operator=(const GlmAsrModel&) = delete;

  std::unique_ptr<Context> create_context(int n_ctx = 0) override;

  Token sot_token_id() const override { return audio_token_id_; }
  ...
  std::vector<Token> eos_token_ids() const override { return eos_token_ids_; }
  bool supports_language_detection() const override { return false; }
  Token audio_token_id() const { return audio_token_id_; }
 ...
 protected:
  ...
  Token audio_token_id_ = 59260;
  std::vector<Token> eos_token_ids_;
