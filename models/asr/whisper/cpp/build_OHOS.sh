#!/usr/bin/env bash
# Build script for Whisper ASR demo on OpenHarmony (OHOS)
#
# Prerequisites:
...
# ------------------------------------------------------------------
# Build houmo-llm-engine (if not already built for OHOS)
# ------------------------------------------------------------------
WORK_PATH="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
HM_ENGINE_DIR="$HOUMO_EXAMPLES_PATH/utils/cpp/houmo_engine"
HM_ENGINE_LIB="$HOUMO_EXAMPLES_PATH/utils/ohos/libhoumo_infer.so"

if [ ! -f "$HM_ENGINE_LIB" ]; then
  echo "[INFO] Building houmo-llm-engine for OHOS..."
  if [ ! -x "$HM_ENGINE_DIR/build_OHOS.sh" ]; then
    echo "Error: $HM_ENGINE_DIR/build_OHOS.sh not found"
    exit 1
  fi
  cd "$HM_ENGINE_DIR"
  ./build_OHOS.sh
  ...
  echo "[INFO] houmo-llm-engine already built, skipping..."
fi
...
echo "  Binary: $WORK_PATH/../bin/demo"
echo "================================================"
