...
if should_run_step "quant"; then
    ...
    python3 ptq.py --model-name "${MODEL_NAME}" --model-size "${MODEL_SIZE}" --quant-type "${QUANT_TYPE}"
fi
...
if should_run_step "demo"; then
    ...
    echo "Execute demo."
    python3 demo.py --model_name "${MODEL_NAME}" --model_size "${MODEL_SIZE}" --device_monitor
fi
