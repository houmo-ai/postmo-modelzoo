...
def attention_rescoring(
    ...
    for b in range(batch_size):
        ...
        hyps_pad = torch.nn.functional.pad(hyps_pad, ( 0, 32 - hyps_pad.shape[1], 0, 0), mode='constant', value=model.ignore_id)
...
def attention_rescoring_xh1(
    ...
    results = []
    for b in range(batch_size):
        encoder_out = encoder_outs[b, :, :].unsqueeze(0)
        encoder_lens[b]
        encoder_mask = torch.zeros(encoder_out.size(1))
        encoder_mask[encoder_lens[b]:] = -65504
        encoder_mask = encoder_mask.view(1, 1, 1, -1).to(encoder_out)
        hyps = ctc_prefix_results[b].nbest
        ctc_scores = ctc_prefix_results[b].nbest_scores
        hyps_pad = pad_sequence([
            torch.tensor(hyp, device=device, dtype=torch.long) for hyp in hyps
        ...
        if getattr(model, 'special_tokens', None) is not None \
            ...
            hyps_lens = hyps_lens + 1  # Add <sos> at begining
            prefix_len = 1
            hyps_pad = torch.nn.functional.pad(hyps_pad, (0, 32 - hyps_pad.shape[1], 0, 0), mode='constant', value=eos)
        decoder_out, r_decoder_out = model.forward_attention_decoder_onnx(
            hyps_pad, hyps_lens, encoder_out, encoder_mask, reverse_weight, attention_rescore_calib_dir)
        
        # from wenet.utils.ln2rms import decoder_ln2rms
        # decoder_ln2rms(model.decoder.left_decoder)
        # decoder_ln2rms(model.decoder.right_decoder)
        # decoder_out, r_decoder_out = model.forward_attention_decoder_onnx(
        #     hyps_pad, hyps_lens, encoder_out, encoder_mask, reverse_weight)
        # Only use decoder score for rescoring
        best_score = -float('inf')
        best_index = 0
        confidences = []
