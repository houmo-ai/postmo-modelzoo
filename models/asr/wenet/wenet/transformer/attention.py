...
class MultiHeadedAttention(nn.Module):
    ...
    def forward_attention_export_onnx(
        self,
        value: torch.Tensor,
        scores: torch.Tensor,
        mask: torch.Tensor = torch.ones((0, 0, 0), dtype=torch.bool),
        mask_pad:  torch.Tensor = None,
    ) -> torch.Tensor:
        """Compute attention context vector.
    ...
    def forward_export_onnx(
        ...
        if self.fuse_sqrt:
            self.linear_q.weight.data = self.linear_q.weight.data / math.sqrt(self.d_k)
            if self.linear_q.bias.data is not None:
                self.linear_q.bias.data = self.linear_q.bias.data / math.sqrt(self.d_k)
            self.fuse_sqrt = False
        q, k, v = self.forward_qkv(query, key, value)
        scores = torch.matmul(q, k.transpose(-2, -1)) 
        return self.forward_attention_export_onnx(v, scores, mask)
...
class RelPositionMultiHeadedAttention(MultiHeadedAttention):
    ...
    def forward_export_onnx(
        ...
        if self.fuse_sqrt:
            ...
            self.pos_bias_u.data = self.pos_bias_u.data / math.sqrt(self.d_k)
            self.pos_bias_v.data = self.pos_bias_v.data / math.sqrt(self.d_k)
            self.fuse_sqrt = False
        ...
        if not self.use_sdpa:
            ...
            if not self.fuse_sqrt:
                scores = (matrix_ac + matrix_bd)  # (batch, head, time1, time2)
            else:
                scores = (matrix_ac + matrix_bd) / math.sqrt(
                    self.d_k)  # (batch, head, time1, time2)
            return self.forward_attention_export_onnx(v, scores, mask, mask_pad)
        else:
           raise NotImplementedError("Not implemented for onnx export")
    ...
    def forward_chunk_export_onnx(
        ...
        pos_emb: torch.Tensor = torch.empty(0),
        mask_pad: torch.Tensor = torch.ones((0, 0, 0), dtype=torch.bool),
        k_catch: torch.Tensor = torch.zeros((0, 0, 0, 0)),
        v_catch: torch.Tensor = torch.zeros((0, 0, 0, 0)),
        offset_start: int = 0,
        offset_end: int = 0,
    ) -> Tuple[torch.Tensor, T_CACHE]:
        """Compute 'Scaled Dot Product Attention' with rel. positional encoding.
        ...
        q = q.transpose(1, 2)  # (batch, time1, head, d_k)
        q_size = q.shape[1]
        k_catch = torch.cat([k_catch[:, :, :offset_start], k, k_catch[:, :, offset_end:]], dim=2)
        v_catch = torch.cat([v_catch[:, :, :offset_start], v, v_catch[:, :, offset_end:]], dim=2)
        # k_catch[:, :, offset:offset + q_size] = k
        # v_catch[:, :, offset:offset + q_size] = v
        k = k_catch
        v = v_catch    
        n_batch_pos = pos_emb.size(0)
        p = self.linear_pos(pos_emb).view(n_batch_pos, -1, self.h, self.d_k)
        ...
        if not self.use_sdpa:
            ...
            if not self.fuse_sqrt:
                ...
                scores = (matrix_ac + matrix_bd) / math.sqrt(
                    self.d_k)  # (batch, head, time1, time2)
            return self.forward_attention_export_onnx(v, scores, mask, mask_pad), k_catch, v_catch
        else:
           raise NotImplementedError("Not implemented for onnx export")
...
class MultiHeadedCrossAttention(MultiHeadedAttention):
    ...
    def forward_export_onnx(
        ...
        scores = torch.matmul(q, k.transpose(-2, -1)) 
        output = self.forward_attention_export_onnx(v, scores, mask)
        return output
