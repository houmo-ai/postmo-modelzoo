...
class TransformerDecoder(torch.nn.Module):
    ...
    @staticmethod
    def modify_linear(linear: torch.nn.Linear, xscale):
        linear.weight.data /= xscale
        if linear.bias is not None:
            linear.bias.data /= xscale
    
    def forward_export_onnx(self, 
                            memory: torch.Tensor, 
                            memory_mask: torch.Tensor, 
                            ys_in_pad: torch.Tensor,
                            ys_in_mask: torch.Tensor,):
        """Forward decoder."""
        tgt = ys_in_pad
        tgt_mask = ys_in_mask
        
        if self.embed[1].xscale != 1:    
            xscale = self.embed[1].xscale
            self.embed[1].pe.data /= xscale
            self.embed[1].xscale = 1
            
            for layer in self.decoders:
                self.modify_linear(layer.self_attn.linear_out, xscale)
                self.modify_linear(layer.src_attn.linear_out, xscale)
                self.modify_linear(layer.feed_forward.w_2, xscale)
        
        x, _ = self.embed(tgt)
        x = self.forward_layers_export_onnx(x, tgt_mask, memory, memory_mask)
        if self.normalize_before:
            x = self.after_norm(x)
        if self.use_output_layer:
            x = self.output_layer(x)
    ...
    def forward_layers_export_onnx(self, x: torch.Tensor, tgt_mask: torch.Tensor,
                       memory: torch.Tensor,
                       memory_mask: torch.Tensor) -> torch.Tensor:
        for layer in self.decoders:
            x = layer.forward_export_onnx(x, tgt_mask, memory,
                                                     memory_mask)
        return x
    ...
    def forward_export_onnx(
        ...
        Returns:
            (tuple): tuple containing:
                ...
                olens: (batch, )
        """
        if hasattr(self, "onnx_left_decoder_model"):
            left_input = dict(
                encoder=memory.detach().cpu().numpy(),
                encoder_mask=memory_mask.detach().cpu().numpy(),
                beam_out=ys_in_pad.detach().cpu().numpy(),
                beam_out_mask=ys_in_mask.detach().cpu().numpy(),
            )
            l_x = self.onnx_left_decoder_model.forward(left_input)
            l_x = torch.from_numpy(l_x["output"]).to(memory)
        else:
            l_x = self.left_decoder.forward_export_onnx(memory, memory_mask, ys_in_pad,
                                            ys_in_mask)
        r_x = torch.tensor(0.0)
        if reverse_weight > 0.0:
            if hasattr(self, "onnx_right_decoder_model"):
                right_input = dict(
                    encoder=memory.detach().cpu().numpy(),
                    encoder_mask=memory_mask.detach().cpu().numpy(),
                    beam_out=r_ys_in_pad.detach().cpu().numpy(),
                    beam_out_mask=ys_in_mask.detach().cpu().numpy(),
                )
                r_x = self.onnx_right_decoder_model.forward(right_input)
                r_x = torch.from_numpy(r_x["output"]).to(memory)
            else:
                r_x = self.right_decoder.forward_export_onnx(memory, memory_mask,
                                                r_ys_in_pad, ys_in_mask)
        return l_x, r_x, None
