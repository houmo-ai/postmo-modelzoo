...
class ASRModel(torch.nn.Module):
    ...
    def _forward_encoder_xh1(
        self,
        speech: torch.Tensor,
        masks_attn: torch.Tensor,
        masks_cnn: torch.Tensor,
    ):
        encoder_out = self.encoder.forward_export_onnx(speech, masks_attn, masks_cnn)
        return encoder_out, masks_cnn
    ...
    @torch.jit.unused
    def ctc_logprobs_onnx(self,
                     encoder_out: torch.Tensor,
                     blank_penalty: float = 0.0,
                     blank_id: int = 0):
        if blank_penalty > 0.0:
            logits = encoder_out
            encoder_out[:, :, blank_id] -= blank_penalty
            ctc_probs = logits.log_softmax(dim=2)
        else:
            ctc_probs = self.ctc.log_softmax_onnx(encoder_out)
        return ctc_probs
    ...
    def decode_xh1(
        ...
        length_penalty: float = 0.0,
        infos: Dict[str, List[str]] = None,
        attention_rescore_calib_dir = None
    ) -> Dict[str, List[DecodeResult]]:
        """ Decode input speech
        ...
        # assert speech.shape[0] == speech_lengths.shape[0]
        assert decoding_chunk_size != 0
        if hasattr(self, 'onnx_encoder_model'):
            input_dict = {
                "input": speech.detach().cpu().numpy(),
                "mask_attn": masks_attn.detach().cpu().numpy(),
                "mask_cnn": masks_cnn.detach().cpu().numpy()
            }
            encoder_out = self.onnx_encoder_model.forward(input_dict)
            ctc_probs = self.ctc_logprobs_onnx(torch.from_numpy(encoder_out["output"]).to(speech), blank_penalty, blank_id)
            encoder_out = torch.from_numpy(encoder_out["hidden_output"]).to(speech)
            encoder_mask = masks_cnn
        elif hasattr(self, "hmodel_encoder_model"):
            input_dict = {
                "input": speech,
                "mask_attn": masks_attn,
                "mask_cnn": masks_cnn
            }
            encoder_out = self.hmodel_encoder_model(input_dict)[0]
            ctc_probs = self.ctc_logprobs_onnx(encoder_out, blank_penalty, blank_id)
            encoder_mask = masks_cnn
        else:
            encoder_out, encoder_mask = self._forward_encoder_xh1(
                speech, masks_attn, masks_cnn)
            ctc_probs = self.ctc_logprobs(encoder_out, blank_penalty, blank_id)
        encoder_lens = encoder_mask.squeeze(1).sum(1).int()
        results = {}
        if 'attention' in methods:
            results['attention'] = attention_beam_search(
                self, encoder_out, encoder_mask, beam_size, length_penalty,
        ...
        if 'attention_rescoring' in methods:
            ...
            if self.apply_non_blank_embedding:
                encoder_out, _ = self.filter_blank_embedding(
                    ctc_probs, encoder_out)
            results['attention_rescoring'] = attention_rescoring_xh1(
                self, ctc_prefix_result, encoder_out, encoder_lens, ctc_weight,
                reverse_weight, infos, attention_rescore_calib_dir)
        return results
    ...
    @torch.jit.export
    def forward_attention_decoder_onnx(
        self,
        hyps: torch.Tensor,
        hyps_lens: torch.Tensor,
        encoder_out: torch.Tensor,
        ...
        #   >>>         [sos, 4, 8, 9],
        #   >>>         [sos, 2, eos, eos]])
        r_hyps = torch.nn.functional.pad(r_hyps, (0, 32 - r_hyps.shape[1], 0, 0), mode='constant', value=2)
        maxlen = hyps.size(1)
        # tgt_mask: (B, 1, L)
        tgt_mask = ~make_pad_mask(hyps_lens, maxlen).unsqueeze(1)
        tgt_mask = tgt_mask.to(hyps.device)
        # m: (1, L, L)
        m = subsequent_mask(tgt_mask.size(-1),
                            device=tgt_mask.device).unsqueeze(0)
        # tgt_mask: (B, L, L)
        tgt_mask = (tgt_mask & m).float()
        tgt_mask = torch.zeros_like(tgt_mask).masked_fill_(~tgt_mask.bool(), -65504).unsqueeze(1)
        if attention_rescore_calib_dir is not None:
            os.makedirs(attention_rescore_calib_dir, exist_ok=True)
            input_dict = {
                "encoder_out": encoder_out.detach().cpu(),
                "encoder_mask": encoder_mask.detach().cpu(),
                "hyps": hyps.detach().cpu(),
                "tgt_mask": tgt_mask.detach().cpu(),
                "r_hyps": r_hyps.detach().cpu(),
            }
            idx = len(os.listdir(attention_rescore_calib_dir)) + 1
            torch.save(input_dict, os.path.join(attention_rescore_calib_dir, f"input_{idx}.pth")) 
        decoder_out, r_decoder_out, _ = self.decoder.forward_export_onnx(
            encoder_out, encoder_mask, hyps, tgt_mask, r_hyps,
            reverse_weight)  # (num_hyps, max_hyps_len, vocab_size)
        decoder_out = torch.nn.functional.log_softmax(decoder_out, dim=-1)
