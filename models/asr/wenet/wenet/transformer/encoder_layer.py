...
class ConformerEncoderLayer(nn.Module):
    ...
    def forward_export_onnx(
        ...
        if self.feed_forward_macaron is not None:
            ...
            if self.ff_scale != 1.0:
                self.feed_forward_macaron.w_2.weight.data *= self.ff_scale
                self.feed_forward_macaron.w_2.bias.data *= self.ff_scale
                
                self.feed_forward.w_2.weight.data *= self.ff_scale
                self.feed_forward.w_2.bias.data *= self.ff_scale
                self.ff_scale = 1.0
        ...
        if self.normalize_before:
            x = self.norm_mha(x)
        x_att = self.self_attn.forward_export_onnx(x, x, x, mask, pos_emb, mask_pad)
        x = residual + self.dropout(x_att)
        if not self.normalize_before:
            x = self.norm_mha(x)
        ...
        if self.conv_module is not None:
            ...
            if self.normalize_before:
                x = self.norm_conv(x)
            x, new_cnn_cache = self.conv_module.forward_export_onnx(x, mask_pad, new_cnn_cache)
            x = residual + self.dropout(x)
    ...
    def forward_chunk_export_onnx(
        ...
        attn_k_cache: torch.Tensor,
        attn_v_cache: torch.Tensor,
        ...
        Args:
            ...
            pos_emb (torch.Tensor): _description_
            mask_pad (torch.Tensor): _description_
            attn_k_cache (torch.Tensor): _description_
            attn_v_cache (torch.Tensor): _description_
            cnn_cache (torch.Tensor): _description_

        Returns:
            Tuple[torch.Tensor, torch.Tensor, T_CACHE, torch.Tensor]: _description_
        """
        # whether to use macaron style
        if self.feed_forward_macaron is not None:
            residual = x
        ...
        if self.normalize_before:
            x = self.norm_mha(x)
        x_att, attn_k_cache, attn_v_cache = self.self_attn.forward_chunk_export_onnx(x, x, x, mask, pos_emb, mask_pad, attn_k_cache, attn_v_cache, offset_start, offset_end)
        x = residual + self.dropout(x_att)
        if not self.normalize_before:
            x = self.norm_mha(x)
        ...
        if self.conv_module is not None:
            ...
            if self.normalize_before:
                x = self.norm_conv(x)
            x, cnn_cache = self.conv_module.forward_chunk_export_onnx(x, mask_pad, cnn_cache)
            x = residual + self.dropout(x)
        ...
        return x, attn_k_cache, attn_v_cache, cnn_cache
