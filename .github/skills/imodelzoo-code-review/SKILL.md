...
会导致数据破坏、敏感信息泄露、广泛安全/稳定性问题，使仓库提供的标准用户入口确定性不可用，或使模型/API 示例目录与仓库根 `README.md` 的 `## 模型示例` / `## API 示例` 登记脱节。
...
- 新增或修改仍在支持中的模型/API 示例目录，但仓库根 `README.md` 对应章节未登记该示例；已标明停止维护的模型适用后文例外。
- 删除仍在支持中的模型/API 示例文件夹，但根 `README.md` 对应章节仍保留该示例描述；已标明停止维护的模型不适用模型目录存在性要求。
...
- 模型或 API 示例目录新增、修改或删除时，必须按“根 README 示例登记与删除同步”规则检查仓库根 `README.md`。

### 根 README 示例登记与删除同步（P0）

仓库根 `README.md` 的 `## 模型示例` 和 `## API 示例` 是仓库级示例登记的唯一基准。本规则只针对实际示例目录变更触发，不因 `data/**`、`apis/data/**`、`apis/models/**` 等 Review Exclusions 路径单独触发。

模型表的 `support` 列是模型目录存在性判断的前置条件：`✅️`（或等价的持续支持标记）表示当前支持；版本号或明确的停止维护文本（例如 `v1.3.0`、`v1.4.0`）表示该模型已停止维护，版本号是最后支持版本。对于已停止维护的模型，README 可以保留历史登记，即使对应目录不再位于 `models/` 下，也不要仅因目录缺失或历史登记残留报告 P0。

- 变更涉及 `models/<category>/<example>/` 的新增或修改时，读取根 `README.md` 的 `## 模型示例` 表。按仓库实际模型/示例标识确认该模型已列出；必要时交叉核对目录名、模型 README 名称和聚合清单，不要只机械比较任意内部脚本名。若对应 README 行的 `support` 已标明停止维护，则不要求该模型当前仍存在于 `models/`；除此之外未列出直接报告 P0。
- 当本次变更新增根 `README.md` 的模型示例登记行时，必须检查 `## 模型示例` 表的排序：新增行应按实际目录路径 `models/<category>/<example>/`，并在同一路径/分类下按模型名称顺序插入；如果周边已有登记顺序与实际路径不一致，要求在同一变更中调整到与实际目录路径一致。未按路径和模型名顺序登记会使后续维护者难以定位示例，按影响报告 P2；若排序错误同时造成模型登记缺失、重复或路径错误，则按登记缺失/错误规则升级为 P0。
- 变更涉及 `apis/converts/<example>/`、`apis/inferences/<example>/` 或 `apis/scenes/<example>/` 的新增或修改时，读取根 `README.md` 的 `## API 示例` 表，确认对应 API 示例已列出。未列出直接报告 P0。
- 删除模型或 API 示例文件夹时，检查根 `README.md` 对应章节是否同步删除该行或描述。对仍在支持中的模型和所有 API 示例，残留登记直接报告 P0；对 `support` 已标明停止维护的模型，允许 README 保留历史登记，不因该登记残留报告 P0。
- 登记缺失或删除未同步不是一般文档漂移，不要降为 P2；但上述已停止维护模型的目录缺失或历史登记残留属于明确例外。finding 锚定到最窄的变更路径或仍残留的 README 行，明确“根 README 登记缺失”或“删除未同步”，并给出最小修复方向：补登记、更正标识，或删除残留描述。
- 仅改测试、聚合配置、HMATC 内部实现或排除路径，且没有对应示例目录新增/修改/删除时，不要凭空报告本规则。
...
10. 若未排除变更包含模型或 API 示例目录的新增、修改或删除，读取仓库根 `README.md` 的 `## 模型示例` 或 `## API 示例`，按“根 README 示例登记与删除同步”规则检查登记缺失和删除残留。
11. 只根据评审上下文中的具体代码证据形成 finding，不用运行时猜测填补证据空白。
...
- 仍在支持中的模型/API 示例目录已新增或修改，但根 `README.md` 对应章节未列出该示例；或示例文件夹已删除，但对应章节仍保留描述。已标明停止维护的模型不因 `models/` 目录缺失或 README 历史登记残留命中本条。
