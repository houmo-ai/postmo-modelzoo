...
- 新增或修改模型/API 示例目录，但仓库根 `README.md` 对应章节未登记该示例。
- 删除模型/API 示例文件夹，但根 `README.md` 对应章节仍保留该示例描述。
...
- 变更涉及 `models/<category>/<example>/` 的新增或修改时，读取根 `README.md` 的 `## 模型示例` 表。按仓库实际模型/示例标识确认该模型已列出；必要时交叉核对目录名、模型 README 名称和聚合清单，不要只机械比较任意内部脚本名。未列出直接报告 P0。
...
- 删除模型或 API 示例文件夹时，检查根 `README.md` 对应章节是否同步删除该行或描述。残留登记直接报告 P0。
- 登记缺失或删除未同步不是一般文档漂移，不要降为 P2。finding 锚定到最窄的变更路径或仍残留的 README 行，明确“根 README 登记缺失”或“删除未同步”，并给出最小修复方向：补登记、更正标识，或删除残留描述。
...
- 模型/API 示例目录已新增或修改，但根 `README.md` 对应章节未列出该示例；或示例文件夹已删除，但对应章节仍保留描述。
