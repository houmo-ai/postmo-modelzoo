...
5. 路径应以文档声明的执行目录或显式配置为基准。不要依赖个人工作目录、个人绝对路径或未说明的环境变量。判定变量未定义前，先检查仓库根 `env.sh`、适用平台环境脚本、父级入口和顶层 README；由标准初始化流程导出的仓库级变量属于已声明 contract，不要求每个子脚本重复定义。

6. 对环境变量问题使用“初始化后值”而不是“当前文件局部定义”作为判断依据。只有标准环境准备后变量仍为空/错误、在到达使用点前被覆盖或清除、目标路径与仓库布局矛盾，或某个公开独立入口明确不要求初始化却依赖该变量时，才能形成 finding。根 `env.sh` 已导出的 `HOUMO_EXAMPLES_PATH` 不能仅因子目录 `run.sh` 未赋值而被视为未定义。
...
1. 对本次新增或修改的功能标识符执行跨文件联合检查。以变量名、环境变量、CLI option、配置 key、对象字段或函数参数为锚点，打开其定义、默认值、所有 changed/directly-coupled 赋值和覆盖、序列化/透传、条件判断、最终消费者、测试与 README。不要在第一个使用文件处停止。

2. 为相关变量建立最小状态表，检查每个状态是否有独立业务含义以及是否可达。若两个变量组合理论上有四种状态，但实现只需要“未提供/已提供”两种状态，应检查额外组合是否造成值被忽略、空值被传递、默认值漂移或调用方必须成对更新。

3. 重点识别派生状态和镜像状态：`*_set`/`has_*`/`enable_*` 与对应 payload，手工维护的 `count` 与集合，缓存副本与权威配置，多个 boolean 与单一 mode，以及 Shell、JSON、Python/C++ 中重复保存的同一默认值。如果状态可从权威值无歧义推导，优先只保留权威值。

4. 以 `SYSTEM_PROMPT` 和 `SYSTEM_PROMPT_SET` 为例：若功能只区分“未提供 prompt”和“提供非空 prompt”，且没有 caller、文档或测试要求“显式空 prompt”具有独立语义，则使用两个可独立设置的变量会产生 `SYSTEM_PROMPT` 非空但 `SYSTEM_PROMPT_SET=false` 时静默忽略输入等矛盾状态，应形成具体 finding。若确实需要区分 unset、empty 和 non-empty，则不能武断删除 flag，而应检查三态 contract 是否有准确命名、校验、文档和测试。

5. 检查新增 wrapper、helper、配置层或中间变量是否承担转换、校验、兼容或复用职责。若只是逐层原样透传、复制默认值或把一个事实重新编码为第二个状态，检查是否增加调用方遗漏和跨文件漂移；没有具体后果的代码精简建议不作为 finding。

6. 跨文件设计 finding 必须引用引入重复状态或错误 contract 的最窄 changed line，并说明关联文件中的消费者证据、可触发组合和用户行为后果。不要只写“设计不优雅”“可以合并变量”或笼统要求重构。
...
2. 如果静态证据表明仓库自带的 `test.sh`、主 `run.sh`、README Quick Start 或其直接调用的公共入口在完成仓库文档要求的 `source env.sh`、平台环境配置及其他必要前置步骤后，仍会在声明支持的参数下确定性失败，按通用规则定为 P0；不能因为修复只需改一个 option、用户可以手工改命令或故障只发生在某个 `test.sh` 阶段而降级。统一环境初始化属于正常前提，不能被当作手工绕过，也不能忽略后虚构“变量未定义”的 P0。
