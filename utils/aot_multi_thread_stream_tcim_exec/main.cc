...
int read_file(char *fileName, char **fileData, int *fileLen) 
{
...
int write_file(char *fileName, char *fileData, int fileLen) 
{
...
bool ParseArgs(CliArguments *arguments, int argc, char *argv[]) {
  ...
  while (true) {
    int ch = getopt_long(argc, argv, "hm:w:i:l:t:", long_options, &option_idx);
    if (ch == -1) {
      break;
    }
    switch (ch) {
...
int main(int argc, char *argv[]) {
  ...
  std::vector<std::vector<std::string>> input_names(arguments.threads);
  ...
  char *imageData = NULL;
	int fileLen = 0;
  // 640x384_422sp.yuv
  read_file((char *)"data_input.bin", &imageData, &fileLen);
  ...
  for (int i = 0; i < arguments.threads; i++) {
    ...
    for (auto input_name : input_names[i]) {
      tvm::runtime::NDArray data = modules[i].GetInputByName(input_name);
      auto input_data = tvm::runtime::NDArray::Empty(
        data.Shape(), data.DataType(), {kDLCPU, 0});
      memcpy(input_data->data, imageData, fileLen);
      modules[i].SetInput(input_name, input_data);
    ...
    for (int idx = 0; idx < tvm_output_count; idx++) {
      ...
      tvm::runtime::NDArray data = modules[i].GetOutputByName(output_name);
...
#if SET_OUTPUT
      module.SetOutput(output_name, output_data);
#endif
      output_datas[i].push_back(output_data);
      int data_size = 1;
      std::cout << "Output " << output_name << ": (";
      for (size_t shape_idx = 0; shape_idx < output_data.Shape().size(); shape_idx++) {
        if (shape_idx != 0) {
          std::cout << ", ";
        }
      ...
      output_data_size[i].push_back(data_size*4);
    ...
    for (int idx = 0; idx < tvm_output_count; idx++) {
      auto output_name = modules[i].GetOutputNameByIndex(idx);
#if !SET_OUTPUT
      output_datas[i][idx] = modules[i].GetOutputByName(output_name);
#endif
      snprintf(fileName, sizeof(fileName), "%s_output.bin", output_name.c_str());
      // write_file(fileName, (char*)output_datas[i][idx]->data, output_data_size[i][idx]);
    }
#endif
  }
  ...
  auto main_loop = [](int t_id, int loop, int interloop, tvm::hdpl::Module &module, std::vector<int> &datasize, int64_t &total_time) {
    ...
    for (int i = 0; i < loop; i++) {
      ...
      for (int idx = 0; idx < tvm_output_count; idx++) {
        ...
        tvm::runtime::NDArray output_data = module.GetFloatOutputByName(output_name);
        ...
        printf("output %s\n", output_name.c_str());
        for (int i = 0; i < 10; i++) {
          printf("%d,", ((float*)(output_data->data))[i]);
        }
      }
#endif
    }
