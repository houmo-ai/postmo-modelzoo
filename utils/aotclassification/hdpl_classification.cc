...
bool HDPLRuntime_GetInputInfo(InputInfo *input_info,
  ...
  if (tvm_input_count != 1) {
    LOG(ERROR) << "Input number is not 1 in the model, but " << tvm_input_count;
    return false;
  }
  input_info->input_name = module->GetInputNameByIndex(0);
...
void HDPLRuntime_SetParam(
  ...
  module->SetInput(input_name, input_array, "YUV422SP");
}
