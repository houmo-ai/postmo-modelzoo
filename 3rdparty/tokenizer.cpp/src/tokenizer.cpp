...
class OnigRegex {
 public:
  OnigRegex(const std::string& pattern) : regex_(nullptr), valid_(false) {
    ...
    OnigErrorInfo einfo;
    onig_init();
    int r = onig_new(&reg, (uint8_t*)pattern.c_str(),
                     (uint8_t*)(pattern.c_str() + pattern.length()),
  ...
  bool search(const std::string& text, int start_offset, int end_offset,
              int& match_start, int& match_end) const {
    ...
    int r = onig_search((regex_t*)regex_, str, str + text.length(), start, end,
...
class NFKCNormalizer : public Normalizer {
 public:
  std::string normalize(const std::string& text) const override {
    ...
    ssize_t ret =
        utf8proc_map((const uint8_t*)text.c_str(), 0, &result,
...
class ReplaceNormalizer : public Normalizer {
 ...
 public:
  ReplaceNormalizer(const std::string& p, const std::string& c)
      : pattern_(p), content_(c) {}
  std::string normalize(const std::string& text) const override {
...
class SequenceNormalizer : public Normalizer {
 ...
 public:
  SequenceNormalizer(const std::vector<std::shared_ptr<Normalizer>>& n)
      : normalizers_(n) {}
  std::string normalize(const std::string& text) const override {
    std::string out = text;
...
class BertNormalizer : public Normalizer {
 ...
 public:
  BertNormalizer(bool clean = true, bool chinese = true, bool accents = false,
                 bool lower = true)
      : clean_text_(clean),
        handle_chinese_chars_(chinese),
        strip_accents_(accents),
        lowercase_(lower) {}
  ...
  std::string normalize(const std::string& text) const override {
    ...
    while (i < len) {
      ...
      if (clean_text_) {
        ...
        if (cp == 0 || cp == 0xFFFD ||
            utf8proc_category(cp) == UTF8PROC_CATEGORY_CC) {
          i += r;
          continue;
        }
      }
      ...
      if (strip_accents_) {
        ...
        ssize_t dlen = utf8proc_map(ptr + i, r, &decomposed,
...
class SequencePreTokenizer : public PreTokenizer {
 public:
  ...
  SequencePreTokenizer(const std::vector<std::shared_ptr<PreTokenizer>>& pts)
      : pts_(pts) {}
...
class ByteLevelPreTokenizer : public PreTokenizer {
 ...
 public:
  ByteLevelPreTokenizer(bool use_regex = false) : use_regex_(use_regex) {
    if (use_regex_) {
      regex_ = std::make_shared<OnigRegex>(
    ...
    if (use_regex_ && regex_ && regex_->is_valid()) {
      ...
      for (const auto& s : pts.splits) {
        ...
        while (last_pos < (int)s.size()) {
          ...
          if (regex_->search(s, last_pos, (int)s.size(), match_start,
                             match_end)) {
            if (match_start > last_pos) {
              next_splits.push_back(s.substr(last_pos, match_start - last_pos));
            }
            if (match_end > match_start) {
              next_splits.push_back(
                  s.substr(match_start, match_end - match_start));
            }
            last_pos = match_end;
  ...
  void pre_tokenize(PreTokenizedString& pts) const override {
    ...
    for (const auto& s : pts.splits) {
      ...
      for (size_t i = 0; i < s.length();) {
        int32_t cp;
        int len =
            utf8proc_iterate((const uint8_t*)s.data() + i, s.size() - i, &cp);
        if (len <= 0) break;
...
class MetaspacePreTokenizer : public PreTokenizer {
 public:
  ...
  MetaspacePreTokenizer(const std::string& rep, bool aps)
      : replacement_(rep), add_prefix_space_(aps) {}
  void pre_tokenize(PreTokenizedString& pts) const override {
    for (auto& s : pts.splits) {
...
class SplitPreTokenizer : public PreTokenizer {
 public:
  ...
  SplitPreTokenizer(const std::string& pattern, bool invert,
                    const std::string& behavior = "Isolated")
      : regex_(tokenizer_make_unique<OnigRegex>(pattern)),
        invert_(invert),
        behavior_(behavior) {}
  void pre_tokenize(PreTokenizedString& pts) const override {
    ...
    for (const auto& s : pts.splits) {
      ...
      while (current_pos < (int)s.size()) {
        int match_start = -1, match_end = -1;
        if (regex_->search(s, current_pos, (int)s.size(), match_start,
                           match_end)) {
          // Found a match
          if (invert_) {
            ...
            if (match_start > current_pos) {
              new_splits.push_back(
                  s.substr(current_pos, match_start - current_pos));
            }
          ...
          if (match_start ==
              match_end) {  // Handle zero-width matches to avoid infinite loops
            current_pos++;
          }
        } else {
      ...
      while (i < len) {
        ...
        bool is_whitespace =
            (cp == ' ' || cp == '\t' || cp == '\n' || cp == '\r' ||
             utf8proc_category(cp) == UTF8PROC_CATEGORY_ZS);
        bool is_punctuation =
            utf8proc_category(cp) == UTF8PROC_CATEGORY_PD ||
...
class BPEModel : public Model {
 public:
  ...
  BPEModel(const std::map<std::string, int>& vocab,
           ...
           const std::map<std::string, int>& added_tokens, bool use_byte_level,
           bool byte_fallback)
...
class WordPieceModel : public Model {
 ...
 public:
  WordPieceModel(const std::string& unk = "[UNK]",
                 const std::string& prefix = "##", int max_chars = 100)
      : unk_token_(unk),
  ...
  std::vector<int> tokenize(const std::string& text) const override {
    ...
    if ((int)text.length() > max_input_chars_per_word_) {
      return unk_token_id_ != -1 ? std::vector<int>{unk_token_id_}
                                 : std::vector<int>{};
    }
    std::vector<int> out;
    size_t start = 0;
 ...
 public:
  ...
  std::vector<int> tokenize(const std::string& text) const override {
    ...
    for (size_t i = 1; i <= n; ++i) {
      ...
      for (size_t j = i - 1; j != (size_t)-1 && j >= start_len;
           --j) {  // Iterate backwards from i-1
        ...
        if (it != vocab_.end()) {
          ...
          if (bf_it != vocab_.end()) {
            ...
            score = (unk_token_id_ < (int)scores_.size())
                        ? scores_[unk_token_id_]
                        : -10.0;
          }
        } else {
          continue;
      ...
      // If unreachable, force greedy step with UNK as fallback if everything
      // failed (Only if not byte fallback, or byte fallback failed to match)
      if (best_scores[i] <= -1e17) {
        ...
        if (prev_score > -1e17) {
          double unk_score = (unk_token_id_ < (int)scores_.size())
                                 ? scores_[unk_token_id_]
                                 : -10.0;
...
class ReplaceDecoder : public Decoder {
 ...
 public:
  ReplaceDecoder(const std::string& p, const std::string& c)
      : pattern_(p), content_(c) {}
...
class StripDecoder : public Decoder {
 ...
 public:
  StripDecoder(const std::string& c, int start, int stop)
      : content_(c), start_(start), stop_(stop) {}
  void decode(std::vector<std::string>& tokens) const override {
    ...
    if (stop_ > 0 && !tokens.back().empty()) {
      ...
      if (pos != std::string::npos &&
          pos + content_.length() == tokens.back().length()) {
  ...
  void decode(std::vector<std::string>& tokens) const override {
    for (auto& t : tokens) {
      if (t.length() >= 3 && t.substr(0, 3) == "<0x") {
        int b;
        if (sscanf(t.c_str(), "<0x%02X>", &b) == 1) t = std::string(1, (char)b);
      }
    }
  }
};
...
class WordPieceDecoder : public Decoder {
 ...
 public:
  WordPieceDecoder(const std::string& prefix = "##", bool cleanup = true)
...
class MetaspaceDecoder : public Decoder {
 ...
 public:
  MetaspaceDecoder(const std::string& rep = "▁", bool aps = true)
      : replacement_(rep), add_prefix_space_(aps) {}
  void decode(std::vector<std::string>& tokens) const override {
    for (auto& t : tokens) {
      std::string out;
...
class SequenceDecoder : public Decoder {
 ...
 public:
  SequenceDecoder(const std::vector<std::shared_ptr<Decoder>>& d)
      : decoders_(d) {}
  void decode(std::vector<std::string>& tokens) const override {
...
class CoreDecoder : public Decoder {
 public:
  ...
  void decode(std::vector<std::string>& tokens)
      const override { /* Not used in this design */
  }
};
...
struct PreTrainedTokenizer::Impl {
  ...
  std::vector<int> encode(const PreTrainedTokenizer* public_api,
                          const std::string& text,
                          bool add_special_tokens) const {
    if (text.empty()) return {};
    std::vector<int> input_ids;

    // 1. Identify added tokens in original text (assuming normalized: false for
    // most)
    std::vector<std::pair<std::string, bool>> units;
    size_t last = 0;
    while (last < text.length()) {
      int match_start = -1, match_end = -1;
      if (added_tokens_regex_ &&
          added_tokens_regex_->search(text, (int)last, (int)text.length(),
                                      match_start, match_end)) {
        std::string match_token =
            text.substr(match_start, match_end - match_start);
        ...
        if (at) {
          if (at->lstrip) {
            while (prefix_end > prefix_start &&
                   isspace((unsigned char)text[prefix_end - 1]))
              prefix_end--;
          }
          if (at->rstrip) {
            while (next_start < text.length() &&
                   isspace((unsigned char)text[next_start]))
              next_start++;
          }
        }
        ...
        if (prefix_end > prefix_start)
          units.push_back(
              {text.substr(prefix_start, prefix_end - prefix_start), false});
    ...
    for (const auto& unit : units) {
      if (unit.second) {
        ...
        std::string normalized =
            normalizer_ ? normalizer_->normalize(unit.first) : unit.first;
  ...
  bool load_from_json(PreTrainedTokenizer* public_api, const json& j) {
    if (j.contains("model") && j["model"].is_object()) {
      ...
      if (model_type.empty()) {
        if (j["model"].contains("vocab") && j["model"]["vocab"].is_array()) {
          model_type = "Unigram";
                   ...
                   (j["model"].contains("vocab") &&
                    j["model"]["vocab"].is_object() &&
      ...
      if (model_type == "WordPiece") {
        ...
        std::string prefix =
            j["model"].value("continuing_subword_prefix", "##");
        int max_chars = j["model"].value("max_input_chars_per_word", 100);
        auto wp =
            std::make_shared<WordPieceModel>(unk_token, prefix, max_chars);
        ...
        if (j["model"].contains("merges")) {
          ...
          for (const auto& item : j["model"]["merges"]) {
            ...
            if (!s1.empty() && !s2.empty() && vocab.count(s1) &&
                vocab.count(s2))
        ...
        // If we have a ByteLevelPreTokenizer in the sequence, BPEModel should
        // not do the mapping itself
        ...
        if (j.contains("pre_tokenizer") && j["pre_tokenizer"].is_object()) {
          ...
          else if (pt.value("type", "") == "Sequence" &&
                   pt.contains("pretokenizers")) {
        ...
        auto bpe = std::make_shared<BPEModel>(
            vocab, merges, added_tokens, use_byte_level && !pt_has_byte_level,
            byte_fallback);
        this->model_ = bpe;
      }
    }
    if (j.contains("normalizer") && !j["normalizer"].is_null()) {
      auto create_norm = [&](const json& s) -> std::shared_ptr<Normalizer> {
        ...
        if (type == "Precompiled") {
          ...
          // For GTE/XLM-R, it seems to map ZWJ (\u200d) to space, causing
          // split.
          std::vector<std::shared_ptr<Normalizer>> norms;
          norms.push_back(std::make_shared<NFKCNormalizer>());
          norms.push_back(std::make_shared<ReplaceNormalizer>(
              "\xE2\x80\x8D", " "));  // ZWJ -> Space
        ...
        if (type == "NFKD")
          return std::make_shared<NFKCNormalizer>();  // Approximate with NFKC
                                                      // for now
        if (type == "Replace") {
          std::string p;
        ...
        if (type == "BertNormalizer") {
          ...
          return std::make_shared<BertNormalizer>(
              ...
              s.value("handle_chinese_chars", true), strip_accents, lowercase);
        }
        return nullptr;
      };
    ...
    if (j.contains("decoder") && !j["decoder"].is_null()) {
      auto create_dec = [&](const json& s) -> std::shared_ptr<Decoder> {
        ...
        if (type == "Strip")
          return std::make_shared<StripDecoder>(
              s.value("content", ""), s.value("start", 0), s.value("stop", 0));
        if (type == "WordPiece")
          return std::make_shared<WordPieceDecoder>(s.value("prefix", "##"),
                                                    s.value("cleanup", true));
        if (type == "Metaspace")
          return std::make_shared<MetaspaceDecoder>(
              s.value("replacement", "\u2581"),
              s.value("add_prefix_space", true));
        return nullptr;
      };
    ...
    if (j.contains("pre_tokenizer") && !j["pre_tokenizer"].is_null()) {
      ...
      auto create_pt = [&](const json& s) -> std::shared_ptr<PreTokenizer> {
        ...
        if (type == "Split") {
          ...
          if (!p.empty())
            return std::make_shared<SplitPreTokenizer>(
                p, s.value("invert", false), s.value("behavior", "Isolated"));
        } else if (type == "Metaspace") {
          return std::make_shared<MetaspacePreTokenizer>(
              s.value("str_rep", "▁"), s.value("add_prefix_space", true));
        } else if (type == "ByteLevel") {
          return std::make_shared<ByteLevelPreTokenizer>(
              s.value("use_regex", true));
        } else if (type == "Digits") {
          return std::make_shared<DigitsPreTokenizer>(
              s.value("individual_digits", false));
        } else if (type == "BertPreTokenizer") {
          return std::make_shared<BertPreTokenizer>();
        } else if (type == "WhitespaceSplit") {
          // T5/Albert use WhitespaceSplit but it seems to break Unigram's
          // ability to see spaces. Ignoring it allows Metaspace to run on the
          // full string, producing "_Hello_World", which Unigram then likely
          // segments correctly.
    ...
    if (j.contains("post_processor") && !j["post_processor"].is_null()) {
        ...
        if (s.contains("single")) {
          for (const auto& i : s["single"]) {
            if (i.contains("SpecialToken"))
              steps.push_back(
                  {true, public_api->token_to_id(
                             i["SpecialToken"]["id"].get<std::string>())});
      ...
      if (pp.value("type", "") == "TemplateProcessing")
        ptl(pp);
      else if (pp.value("type", "") == "Sequence" &&
               pp.contains("processors")) {
    ...
    if (j.contains("added_tokens") && j["added_tokens"].is_array()) {
      ...
      for (const auto& item : j["added_tokens"]) {
        ...
        cs.push_back(c);
        this->added_tokens_.push_back({id, c, special, lstrip, rstrip,
                                       normalized});  // Store added token info
    ...
    if (j.contains("config_overrides")) {
      ...
      if (co.contains("bos_token"))
        this->special_tokens_.bos =
            public_api->token_to_id(get_token_content(co["bos_token"]));
      if (co.contains("eos_token"))
        this->special_tokens_.eos =
            public_api->token_to_id(get_token_content(co["eos_token"]));
      if (co.contains("pad_token"))
        this->special_tokens_.pad =
            public_api->token_to_id(get_token_content(co["pad_token"]));
      if (co.contains("unk_token"))
        this->special_tokens_.unk =
            public_api->token_to_id(get_token_content(co["unk_token"]));
    }
    return true;
  }
};
...
std::vector<int> PreTrainedTokenizer::encode(const std::string& text,
                                             bool add_special_tokens) const {
  return impl_->encode(this, text, add_special_tokens);
}

std::string PreTrainedTokenizer::decode(const std::vector<int>& ids,
...
int PreTrainedTokenizer::token_to_id(const std::string& t) const {
  return impl_->model_ ? impl_->model_->token_to_id(t) : -1;
}
std::string PreTrainedTokenizer::id_to_token(int id) const {
  return impl_->model_ ? impl_->model_->id_to_token(id) : "";
}
int PreTrainedTokenizer::pad_token_id() const {
  return impl_->special_tokens_.pad;
}
int PreTrainedTokenizer::bos_token_id() const {
  return impl_->special_tokens_.bos;
}
int PreTrainedTokenizer::eos_token_id() const {
  return impl_->special_tokens_.eos;
}
int PreTrainedTokenizer::unk_token_id() const {
  return impl_->special_tokens_.unk;
}
...
std::string PreTrainedTokenizer::apply_chat_template(const ChatMessages& msgs,
                                                     bool add_gen) const {
  if (!impl_->jinja_template_) return "";
  json j_msgs = json::array();
  for (const auto& m : msgs)
    j_msgs.push_back({{"role", m.first}, {"content", m.second}});
  json extra = json::object();
  extra["bos_token"] = id_to_token(impl_->special_tokens_.bos);
  extra["eos_token"] = id_to_token(impl_->special_tokens_.eos);
  return impl_->jinja_template_->apply_chat_template(j_msgs, add_gen,
                                                     json::array(), extra);
}

std::string PreTrainedTokenizer::apply_chat_template(
    const std::string& json_str, bool add_generation_prompt) const {
  if (!impl_->jinja_template_) return "";
  ...
  extra["bos_token"] = id_to_token(impl_->special_tokens_.bos);
  extra["eos_token"] = id_to_token(impl_->special_tokens_.eos);
  return impl_->jinja_template_->apply_chat_template(
      j_msgs, add_generation_prompt, json::array(), extra);
}
...
std::shared_ptr<PreTrainedTokenizer> AutoTokenizer::from_pretrained(
